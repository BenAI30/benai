...
function logConnexion(uid){
  try{
    ...
    logs.unshift({uid,date:new Date().toLocaleString('fr-FR',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'})});
...
function renderConnexionsHistory(){
  ...
  logs.forEach(l=>{
    ...
    const rl=document.createElement('div');rl.style.cssText='font-size:10px;color:var(--t3)';rl.textContent=u?(ROLE_LABELS[u.role]||''):'';
...
function renderUsersList(){
  ...
  if(search)users=users.filter(u=>(u.name||'').toLowerCase().includes(search)||(u.role||'').includes(search));
  const access=getAccess();
  list.innerHTML='';
  users.forEach(u=>{
...
function renderLeadsDashboard(){
  ...
  html+=`<div class="secteur-card">
    <div class="secteur-title">🧭 Suivi global par secteur</div>
...
function buildCRMExportBundles(){
  ...
  const secteurRows=secteurs.map(s=>{
    const sl=actifs.filter(l=>l.secteur===s);
