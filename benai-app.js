...
let _benaiDlgTail=Promise.resolve();
function benaiDialogQueue(task){
  const run=_benaiDlgTail.then(()=>task());
  _benaiDlgTail=run.catch(()=>{});
  return run;
}
let _benaiDlgResolver=null;
function _benaiDlgClose(val){
  const overlay=document.getElementById('benai-dialog-overlay');
  if(overlay){
    overlay.classList.remove('open');
    overlay.onclick=null;
  }
  document.removeEventListener('keydown',_benaiDlgKeydown,true);
  const r=_benaiDlgResolver;
  _benaiDlgResolver=null;
  window._benaiDlgKind=null;
  if(r)r(val);
}
function _benaiDlgKeydown(e){
  const k=window._benaiDlgKind;
  if(!k)return;
  if(e.key==='Escape'){
    e.preventDefault();
    e.stopPropagation();
    if(k==='alert')_benaiDlgClose();
    else _benaiDlgClose(k==='confirm'?false:null);
  }else if(e.key==='Enter'){
    e.preventDefault();
    e.stopPropagation();
    const ok=document.getElementById('benai-dialog-btn-ok');
    if(ok)ok.click();
  }
}
function _openBenaiDialog(opts){
  return new Promise(resolve=>{
    const overlay=document.getElementById('benai-dialog-overlay');
    const msgEl=document.getElementById('benai-dialog-message');
    const titleEl=document.getElementById('benai-dialog-title');
    const iconEl=document.getElementById('benai-dialog-icon');
    const fieldWrap=document.getElementById('benai-dialog-field');
    const inp=document.getElementById('benai-dialog-input');
    const btnOk=document.getElementById('benai-dialog-btn-ok');
    const btnCancel=document.getElementById('benai-dialog-btn-cancel');
    if(!overlay||!msgEl||!titleEl||!btnOk||!btnCancel){
      resolve(opts.kind==='confirm'?false:opts.kind==='prompt'?null:undefined);
      return;
    }
    if(iconEl)iconEl.textContent=opts.kind==='confirm'?'❔':opts.kind==='prompt'?'✏️':'💬';
    window._benaiDlgKind=opts.kind;
    _benaiDlgResolver=resolve;
    titleEl.textContent=opts.title||'BenAI';
    ...
    if(opts.kind==='prompt'){
      ...
      inp.value='';
      btnCancel.style.display=opts.kind==='confirm'?'':'none';
    }
    const onOk=()=>{
      btnOk.removeEventListener('click',onOk);
      btnCancel.removeEventListener('click',onCancel);
      if(opts.kind==='prompt')_benaiDlgClose(inp.value);
      else if(opts.kind==='confirm')_benaiDlgClose(true);
      else _benaiDlgClose(undefined);
    };
    const onCancel=()=>{
      btnOk.removeEventListener('click',onOk);
      btnCancel.removeEventListener('click',onCancel);
      if(opts.kind==='prompt')_benaiDlgClose(null);
      else if(opts.kind==='confirm')_benaiDlgClose(false);
    };
    btnOk.addEventListener('click',onOk);
    btnCancel.addEventListener('click',onCancel);
    overlay.onclick=e=>{if(e.target===overlay){if(opts.kind==='alert')onOk();else onCancel();}};
    document.addEventListener('keydown',_benaiDlgKeydown,true);
...
function benaiAlert(message,title){
  return benaiDialogQueue(()=>_openBenaiDialog({kind:'alert',message:String(message??''),title:title||'BenAI'}));
}
function benaiConfirm(message,title){
  return benaiDialogQueue(()=>_openBenaiDialog({kind:'confirm',message:String(message??''),title:title||'Confirmation'}));
}
function benaiPrompt(message,defaultValue,title,inputType){
  return benaiDialogQueue(()=>_openBenaiDialog({
    kind:'prompt',
...
const ROLE_GUIDE_ACK_LOCAL_PREFIX='benai_role_guide_ack_local_';
const STORAGE_KEYS = {
  mem:'benai_v3',
  api:'benai_api',
  pwds:'benai_pwds',
...
/** Dernière erreur PostgREST (fetch profiles ou rpc ensure_my_profile) pour diagnostiquer la connexion. */
let lastSupabaseProfileLoginHint='';
...
function pickLatestSav(prev,next){
  const aTs=Number(prev?.sync_ts||0);
  const bTs=Number(next?.sync_ts||0);
  if(bTs>aTs)return next;
  if(aTs>bTs)return prev;
  const aAct=Array.isArray(prev?.actions)?prev.actions.length:0;
  const bAct=Array.isArray(next?.actions)?next.actions.length:0;
  if(bAct>aAct)return next;
  if(aAct>bAct)return prev;
  const aSt=String(prev?.statut||'');
  const bSt=String(next?.statut||'');
  if(aSt!==bSt){
    const rank={regle:3,en_cours:2,nouveau:1};
    if((rank[bSt]||0)>(rank[aSt]||0))return next;
    if((rank[bSt]||0)<(rank[aSt]||0))return prev;
  }
  return next;
}
  ...
  try{
    ...
    if(!res.ok){
      try{
        ...
        lastSupabaseProfileLoginHint=String(j.message||j.details||j.hint||j.code||'').trim().slice(0,220)||('HTTP '+res.status);
      }catch{
        lastSupabaseProfileLoginHint=('HTTP '+res.status).slice(0,220);
      }
      return null;
    }
    const rows=await res.json();
...
/** Crée / met à jour la ligne profiles pour l’utilisateur connecté (RPC SQL ensure_my_profile). */
async function rpcEnsureMySupabaseProfile(accessToken){
  if(!SUPABASE_CONFIG.enabled||!SUPABASE_CONFIG.url||!accessToken)return false;
  try{
    const res=await fetchWithTimeout(`${SUPABASE_CONFIG.url}/rest/v1/rpc/ensure_my_profile`,{
      method:'POST',
      headers:{
        'Content-Type':'application/json',
    ...
    try{
      ...
      lastSupabaseProfileLoginHint=String(j.message||j.error||j.details||j.hint||text).trim().slice(0,220);
    }catch{
      lastSupabaseProfileLoginHint=String(text||'HTTP '+res.status).trim().slice(0,220);
    }
    return false;
  }catch(e){
    lastSupabaseProfileLoginHint=String(e&&e.message||e||'réseau').slice(0,220);
    return false;
  }
}
...
function makeUserFromSupabaseProfile(profile,emailFallback=''){
  ...
  if(roleNorm==='admin'&&/benjamin/i.test(String(profile.full_name||'')))uid='benjamin';
  return {
    id:uid,
    auth_uid:profile.id||'',
    ...
    color:getColorForRole(roleNorm),
...
async function trySupabaseLogin(resolvedEmail,pwd,rawLogin=''){
  ...
  try{
    ...
    let profile=await fetchSupabaseProfile(data.session.access_token,uid);
    if(!profile){
      const repaired=await rpcEnsureMySupabaseProfile(data.session.access_token);
      if(repaired)profile=await fetchSupabaseProfile(data.session.access_token,uid);
    }
...
/** Pilotage → Fichiers & outils : remise à zéro cloud (rôle admin + mot de passe cloud). */
async function pilotageAdminResetDemoData(){
  ...
  const step1=await benaiConfirm(
    'Action définitive : suppression en base de tous les leads, SAV, notes, absences, annuaire, messages partagés, snapshots benai_state, puis suppression de tous les comptes BenAI sauf le vôtre.\n\nLes autres postes devront vider le stockage du site ou se reconnecter.',
  ...
  showDriveNotif('⏳ Remise à zéro en cours (peut prendre une minute)…');
  ...
  showDriveNotif('');
  const n=Number(res.data?.deleted_users);
  const extra=Number.isFinite(n)&&n>=0?`\nComptes supprimés : ${n}.`:'';
  try{
  ...
  await benaiAlert(`Base nettoyée.${extra}\n\nLa page va se recharger.`);
  location.reload();
}
...
async function finalizeBenAILocalUserAccount({uid,name,email,role,soc,fonction,pwd,extras,roleLabel}){
  ...
  const orgHint=(()=>{
    ...
    if(s==='les-deux')return'l’organisation (compte multi-périmètres CRM)';
    if(s==='lambert')return'Lambert SAS';
    return'Nemausus Fermetures';
  })();
  const systemPrompt=CRM_PAGES_ONLY.includes(role)
    ?`Tu es BenAI CRM, assistant de ${name} (${roleLabel}) pour ${orgHint}.`
    :`Tu es BenAI, l'assistant de ${name} (${fonction}) pour ${orgHint}. Tu proposes toujours — ${name} décide toujours.`;
...
async function saveSharedApiKeyToSupabase(rawKey){
  ...
  try{
    ...
    if(res.status===401||res.status===403){
      ...
      let profile=await fetchSupabaseProfile(session.access_token,aid);
      if(!profile&&(await rpcEnsureMySupabaseProfile(session.access_token))){
        profile=await fetchSupabaseProfile(session.access_token,aid);
      }
      if(!profile)return {ok:false,reason:'profile_missing'};
...
async function syncSupabasePostLogin(){
  ...
  if(currentSupabaseSession?.access_token){
    ...
    let profile=await fetchSupabaseProfile(currentSupabaseSession.access_token,uid);
    if(!profile&&(await rpcEnsureMySupabaseProfile(currentSupabaseSession.access_token))){
      profile=await fetchSupabaseProfile(currentSupabaseSession.access_token,uid);
    }
    if(!profile){
      showDriveNotif('⚠️ Profil Supabase manquant : exécutez le script SQL ensure_my_profile (fichier supabase/patch_ensure_my_profile.sql) ou créez la ligne dans public.profiles.');
    }
  }
}
...
async function requestAnthropicMessages(body,retryOnInvalidKey=true,timeoutMs=120000){
...
async function login(){
  ...
  if(isEmailLogin||resolvedSupabaseEmail){
    ...
    if(reason.includes('invalid login credentials')){
      ...
      supabaseFailureLabel='Connexion Supabase OK mais profil manquant dans la table profiles. Réexécutez supabase/patch_ensure_my_profile.sql dans l’éditeur SQL ; si une fenêtre s’affiche, cliquez uniquement sur le bouton vert « Exécuter et activer RLS », puis réessayez.';
      if(lastSupabaseProfileLoginHint)supabaseFailureLabel+=' Détail : '+lastSupabaseProfileLoginHint+'.';
    }else{
...
function openLoginForgotEmailModal(defaultVal,err,onSubmit){
  document.getElementById('login-forgot-email-overlay')?.remove();
  const wrap=document.createElement('div');
  wrap.id='login-forgot-email-overlay';
  wrap.className='lead-modal open';
  wrap.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:12000;display:flex;align-items:center;justify-content:center;padding:16px';
  wrap.innerHTML=`
    <div class="lead-modal-box" style="max-width:420px" role="dialog" aria-modal="true" aria-labelledby="login-forgot-email-title" onclick="event.stopPropagation()">
      <div class="lead-modal-header">
        <div id="login-forgot-email-title" style="font-size:16px;font-weight:700">Réinitialisation du mot de passe</div>
        <button type="button" id="login-forgot-email-close-x" style="background:transparent;border:none;color:var(--t2);font-size:20px;line-height:1;cursor:pointer;padding:4px 8px" aria-label="Fermer">×</button>
      </div>
      <div class="lead-modal-body">
        <p style="font-size:13px;color:var(--t2);margin:0 0 10px;line-height:1.45">Entrez l’adresse e-mail de votre compte Supabase (celle qui reçoit le lien de connexion).</p>
        <div class="form-group">
          <div class="form-label">E-mail</div>
          <input type="email" class="form-input" id="login-forgot-email-input" value="" placeholder="vous@exemple.fr" style="font-family:inherit">
        </div>
        <div id="login-forgot-email-inline-err" style="font-size:12px;color:var(--r);min-height:18px"></div>
        <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:8px;flex-wrap:wrap">
          <button type="button" id="login-forgot-email-cancel" style="padding:10px 16px;background:var(--s3);color:var(--t1);border:1px solid var(--b1);border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit">Annuler</button>
          <button type="button" id="login-forgot-email-send" style="padding:10px 16px;background:linear-gradient(135deg,var(--a),var(--a2));color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">Envoyer le lien</button>
        </div>
      </div>
    </div>`;
  const close=()=>{
  ...
  const errEl=wrap.querySelector('#login-forgot-email-inline-err');
  const inp=wrap.querySelector('#login-forgot-email-input');
  if(inp&&defaultVal)inp.value=defaultVal;
  wrap.querySelector('#login-forgot-email-close-x')?.addEventListener('click',close);
  wrap.querySelector('#login-forgot-email-cancel')?.addEventListener('click',close);
  wrap.querySelector('#login-forgot-email-send')?.addEventListener('click',()=>{
    const v=(inp?.value||'').trim().toLowerCase();
    if(!v||!v.includes('@')){if(errEl)errEl.textContent='Adresse e-mail invalide.';return;}
...
async function proceedForgotPasswordReset(email,err){
  ...
  if(!SUPABASE_CONFIG.enabled||!client){
    ...
    await benaiAlert('Réinitialisation indisponible sur cet appareil.\nContactez un administrateur BenAI.');
    return;
  }
  if(err){
    err.style.color='var(--t2)';
  ...
  try{
    ...
    await benaiAlert(`Un email de réinitialisation a été envoyé à ${email}.\nPensez à vérifier les spams.`);
  }catch(e){
    if(err){
      err.style.color='var(--r)';
...
async function forgotPassword(){
  ...
  if(!email||!email.includes('@')){
    openLoginForgotEmailModal(rawLogin.includes('@')?rawLogin:'',err,v=>void proceedForgotPasswordReset(v,err));
    return;
  }
  await proceedForgotPasswordReset(email,err);
}
...
/** Slug société pour messagerie : DG / dir. co = multi-entité ; terrain sans `societe` = accès interne jusqu’à correction du profil. */
...
/** Après sync REST des profils, réaligner l’utilisateur courant (ex. `societe` depuis Supabase) pour la liste Messages. */
...
async function switchToUser(uid){
  ...
  if(!u){await benaiAlert('Utilisateur introuvable');return;}
  const access=getAccess();
  if(access[uid]===false){await benaiAlert(`Accès bloqué pour ${u.name}`);return;}
...
function refreshSidebarSectionsVisibility(){
  const sidebar=document.querySelector('.sidebar');
  if(!sidebar)return;
  sidebar.querySelectorAll('.sb-section').forEach(sec=>{
...
/** Menu latéral : même structure et le même ordre pour tous les rôles (seules les entrées autorisées s’affichent). */
function applySidebarSectionOrderForRole(role){
  void role;
  const sidebar=document.querySelector('.sidebar');
  const collab=document.getElementById('sb-collab');
  const metier=document.getElementById('sb-metier');
  const org=document.getElementById('sb-org');
  const aide=document.getElementById('sb-aide');
  if(!sidebar||!collab||!metier||!org||!aide)return;
  const collabLbl=collab.querySelector('.sb-label');
  const metierLbl=metier.querySelector('.sb-label');
  const orgLbl=org.querySelector('.sb-label');
  const hintCollab=document.getElementById('sb-collab-hint');
  const hintMetier=document.getElementById('sb-metier-hint');
  const hintOrg=document.getElementById('sb-org-hint');
  if(collabLbl)collabLbl.textContent='Échanger';
  if(metierLbl)metierLbl.textContent='Clients & dossiers';
  if(orgLbl)orgLbl.textContent='Organisation';
  if(hintCollab)hintCollab.textContent='Messages, notes, assistant IA';
  if(hintMetier)hintMetier.textContent='Leads, devis, SAV';
  if(hintOrg)hintOrg.textContent='Équipe, absences, pilotage';
  [collab,metier,org,aide].forEach(sec=>sidebar.appendChild(sec));
}
...
function initApp(silent=false){
  ...
  refreshSidebarSectionsVisibility();
...
async function sendChat(txtOverride){
  ...
  if(!txtOverride&&isBenAIDesktopAutocorrect()){
    const apiKey=await getApiKeyForChat();
    if(apiKey){
      try{
...
function getCRMContextForAI(){
  ...
  const actifs=leads.filter(l=>!isLeadCrmArchivedView(l));
...
function generateLocalCRMReport(){
  ...
  const actifs=scopedLeads.filter(l=>!isLeadCrmArchivedView(l));
  const now=new Date();
...
async function correctSpelling(){
  ...
  if(!txt)return;
  const apiKey=await getApiKeyForChat();if(!apiKey){showDriveNotif('⚠️ Clé API IA indisponible — Paramètres ou synchro équipe');return;}
  try{
...
async function autoCorrectForBenjamin(txt,msgEl){
  if(!isBenAIDesktopAutocorrect())return;
  const apiKey=await getApiKeyForChat();if(!apiKey||!msgEl)return;
  try{
    const res=await fetch('https://api.anthropic.com/v1/messages',{
      method:'POST',
      headers:{'Content-Type':'application/json','x-api-key':apiKey,'anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-access':'true'},
    ...
    const wrapper=msgEl.querySelector('.tmsg-body')||msgEl.querySelector('[style*="flex-direction:column"]')||msgEl;
...
function patchConvListRowForCid(cid){
  ...
  const header=list.querySelector('.conv-header');
  if(header&&item.previousElementSibling!==header){
    try{list.insertBefore(item,header.nextSibling);}catch(_){}
  }
  return true;
}
...
function appendBenAIMotivationInternalMessage(recipientUid,plainText){
  ...
  saveMem(mem);
  try{logActivity(`BenAI a envoyé un message d'encouragement à ${getAllUsers().find(u=>String(u.id)===String(recipientUid))?.name||recipientUid}`);}catch(_){}
  if(String(currentUser?.id)===String(recipientUid)&&document.getElementById('page-messages')?.style.display==='flex'){
    if(currentConv===cid){
...
async function deleteMsg(cid,idx){
  if(currentUser.id!=='benjamin'){showDriveNotif('⚠️ Suppression réservée à Benjamin');return;}
  if(!await benaiConfirm('Supprimer ce message définitivement ?'))return;
  const mem=getMem();
...
function editMsg(cid,idx){
  ...
  document.getElementById('edit-msg-overlay')?.remove();
  const wrap=document.createElement('div');
  wrap.id='edit-msg-overlay';
  wrap.className='lead-modal open';
  wrap.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:4500;display:flex;align-items:center;justify-content:center;padding:16px';
  wrap.innerHTML=`
    <div class="lead-modal-box" style="max-width:520px;width:100%" role="dialog" aria-modal="true" aria-labelledby="edit-msg-title" onclick="event.stopPropagation()">
      <div class="lead-modal-header">
        <div id="edit-msg-title" style="font-size:16px;font-weight:700">Modifier le message</div>
...
/** Évite de re-render la liste SAV à chaque touche dans la recherche (sinon les champs « action » disparaissent). */
function scheduleRenderSAVFromSearch(){
  if(savSearchDebounceTimer)clearTimeout(savSearchDebounceTimer);
  savSearchDebounceTimer=setTimeout(()=>{
    savSearchDebounceTimer=null;
    renderSAV();
  },320);
}

/** Sauvegarde les brouillons saisis dans les champs action admin avant re-render (sync cloud, filtres, etc.). */
function captureSavActionDrafts(){
  const out={};
  document.querySelectorAll('input.sav-action-input[id^="action-"]').forEach(el=>{
    const raw=String(el.id||'').replace(/^action-/,'');
    if(!raw)return;
...
function applySavActionDrafts(drafts){
...
async function addSAV(){
  ...
  if(missing.length){
    await benaiAlert('Champs requis ou invalides : '+missing.join(', '));
    return;
  }
  const mem=getMem();
...
function renderSAV(){
  ...
  if(!savs.length){
    list.innerHTML=`<div style="color:var(--t3);font-size:13px;padding:24px;text-align:center">${currentFilter==='regle'?'Aucun SAV réglé':'Aucun SAV en cours 🎉'}</div>`;
    applySavActionDrafts(actionDrafts);
    return;
  }
  ...
  list.innerHTML=savs.map(s=>{
    ...
    return `<div class="sav-card${s.urgent?' urgent':''}">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap">
        ${priorityBadge}
        ${stateLabel[s.statut]||''}
        ${formatSavSocieteBadgeHtml(s.societe)}
      ...
      ${isAdmin&&s.statut!=='regle'?`<div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap">
        <input type="text" class="sav-action-input" id="action-${s.id}" placeholder="Ce que tu as fait (ex: Commandé chez Somfy le 12/04...)" spellcheck="false" autocorrect="off" autocapitalize="off">
...
function getClientHistory(client,excludeId,mem){
  ...
  return (mem.sav||[]).filter(s=>!s._deleted&&s.client===client&&String(s.id)!==ex).slice(0,5);
}

async function deleteSAV(id){
  if(!await benaiConfirm('Supprimer ce SAV définitivement ?'))return;
  const mem=getMem();
  const sid=String(id);
  const sav=mem.sav.find(s=>String(s.id)===sid);
  if(!sav)return;
  sav._deleted=true;
...
function editSAV(id){
  const mem=getMem();const sid=String(id);const s=mem.sav.find(x=>String(x.id)===sid);if(!s)return;
  syncSavSocieteFormOptions();
...
async function saveEditSAV(id){
  ...
  const mem=getMem();const sid=String(id);const idx=mem.sav.findIndex(s=>String(s.id)===sid);if(idx===-1)return;
  mem.sav[idx]={
    ...mem.sav[idx],
    client,probleme:pb,
...
const SAV_STATUT_NOTIF_LABELS={nouveau:'Nouveau',en_cours:'En cours',regle:'Réglé'};

/** Destinataires des notifs SAV (hors auteur du changement) : admin, assistantes de la même société, créateur et commercial si reconnus par le nom. */
function getSavTeamNotifRecipientIds(sav,actorId){
  const ids=[];
  const add=(id)=>{
    if(!id)return;
    const n=normalizeId(String(id));
    if(actorId&&normalizeId(String(actorId))===n)return;
    if(!ids.some(x=>normalizeId(String(x))===n))ids.push(id);
  };
  const soc=String(sav?.societe||'').trim();
  getAllUsers().forEach(u=>{
    if(!u?.id)return;
    if(u.role==='admin'){add(u.id);return;}
    if(u.role==='assistante'&&soc&&(u.societe===soc||u.societe==='les-deux')){add(u.id);}
  });
  const matchNameToId=nameRaw=>{
    const n=String(nameRaw||'').trim().toLowerCase();
    if(!n)return;
    const u=getAllUsers().find(x=>String(x.name||'').trim().toLowerCase()===n);
    if(u)add(u.id);
  };
  matchNameToId(sav.by);
  matchNameToId(sav.commercial);
  return ids;
}

/** Notifie l’équipe d’un changement de statut SAV (flux partagé Supabase si activé). */
function notifySavTeamStatusChange(sav,prevStatut,nextStatut){
  if(!sav||!currentUser)return;
  if(String(prevStatut||'')===String(nextStatut||''))return;
  const labelPrev=SAV_STATUT_NOTIF_LABELS[prevStatut]||prevStatut;
  const labelNext=SAV_STATUT_NOTIF_LABELS[nextStatut]||nextStatut;
  const clientShort=String(sav.client||'').trim().slice(0,120)||'Client';
  const titre=`🔧 SAV · ${labelNext}`;
  const msg=`${clientShort} — ${labelPrev} → ${labelNext} · ${currentUser.name}`;
  const icon='🔧';
  const recipients=getSavTeamNotifRecipientIds(sav,currentUser.id);
  if(!recipients.length)return;
  const now=Date.now();
  const timeStr=new Date().toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'});
  if(SUPABASE_CONFIG.enabled&&SUPABASE_CONFIG.url){
    const feed=recipients.map((uid,i)=>({id:now*1000+i*97+Math.floor(Math.random()*40),target_uid:String(uid),titre,msg,icon,time:timeStr,ts:now}));
    void pushSharedCoreDataToSupabase(getMem(),getAnnuaire(),getLeads(),feed);
  }
  refreshNotifBadge();
}
...
function savAction(id){
  ...
  notifySavTeamStatusChange(sav,prevStatut,'en_cours');
}

function changeStatut(id,statut){
  const mem=getMem();const sid=String(id);const sav=mem.sav.find(s=>String(s.id)===sid);
  if(sav){
    const prevStatut=sav.statut||'nouveau';
    sav.statut=statut;if(statut==='regle')sav.date_reglement=new Date().toLocaleDateString('fr-FR');sav.sync_ts=Date.now();saveMem(mem);renderSAV();refreshSAVBadge();logActivity(`${currentUser.name} a réglé SAV : ${sav.client}`);
    notifySavTeamStatusChange(sav,prevStatut,statut);
  }
}
...
function exportSAVCSV(){
  ...
  const header=joinCsv(['Date','Client','Problème','Société','Commercial','Fournisseur','Statut','Urgent','Date rappel'])+'\n';
  const rows=savs.map(s=>joinCsv([
    s.date_creation||'',
    csv(s.client),
    csv(s.probleme),
    s.societe==='lambert'?'lambert':'nemausus',
    csv(s.commercial),
    csv(s.fournisseur),
    csv(String(s.statut||'')),
    s.urgent?'Oui':'Non',
...
async function correctNoteSpelling(id,el){
  ...
  const apiKey=await getApiKeyForChat();if(!apiKey)return;
  try{
    const r=await fetch('https://api.anthropic.com/v1/messages',{
      method:'POST',
      headers:{'Content-Type':'application/json','x-api-key':apiKey,'anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-access':'true'},
...
async function deleteNote(id){
  if(!await benaiConfirm('Supprimer cette note ?'))return;
  const mem=getMem();
  const n=(mem.notes||[]).find(x=>noteIdEq(x.id,id));
  if(!n)return;
  n._deleted=true;
...
async function deleteAbsence(id){
  if(currentUser?.role!=='admin')return;
  if(!await benaiConfirm('Supprimer cette absence ?'))return;
  const mem=getMem();
...
async function saveEditAbsence(id){
  if(currentUser?.role!=='admin')return;
  const emp=document.getElementById('abs-emp-select')?.value||'';
  const debut=document.getElementById('abs-debut').value;
  const fin=document.getElementById('abs-fin').value;
  if(!emp||!debut||!fin){await benaiAlert('Remplissez tous les champs obligatoires');return;}
  if(new Date(fin)<new Date(debut)){await benaiAlert('⚠️ La date de fin doit être après le début');return;}
  const notifs=buildAbsenceNotifsArray();
...
function resetEmpBenaiProvisionUi(){
  const c=document.getElementById('emp-opt-benai');
  if(c)c.checked=false;
  const fld=document.getElementById('emp-benai-fields');
  if(fld)fld.style.display='none';
  const pw=document.getElementById('emp-benai-pwd');
  if(pw)pw.value=generateBenAIPassword(14);
  const pseudo=document.getElementById('emp-benai-app-uid');
  if(pseudo)pseudo.value='';
  const lm=document.getElementById('emp-benai-linked-msg');
  if(lm){lm.style.display='none';lm.textContent='';}
}
function syncEmpFormBenaiSection(){
  const wrap=document.getElementById('emp-benai-create-wrap');
  if(!wrap)return;
  const admin=canManageBenaiUsersAdmin();
  wrap.style.display=admin?'block':'none';
  if(!admin)return;
  const linkedMsg=document.getElementById('emp-benai-linked-msg');
  const createBlock=document.getElementById('emp-benai-checkbox-block');
  const pseudoRow=document.getElementById('emp-benai-pseudo-row');
  if(pseudoRow)pseudoRow.style.display=canAssignBenaiLoginPseudo()?'block':'none';
  if(editingEmployeId!=null&&editingEmployeHadBenai){
    const emp=(getAnnuaire()||[]).find(x=>x.id===editingEmployeId);
    const u=emp?findBenaiAccountLinkedToAnnuaireEmploye(emp):null;
    if(linkedMsg){
      linkedMsg.style.display='block';
      linkedMsg.innerHTML=u
        ?`Accès BenAI déjà actif : <strong>${esc(ROLE_LABELS[u.role]||u.role)}</strong> · identifiant <code style="font-size:10px">${esc(String(u.id))}</code>`
        :'Accès BenAI déjà lié à cette fiche.';
    }
    if(createBlock)createBlock.style.display='none';
  }else{
    if(linkedMsg)linkedMsg.style.display='none';
...
function toggleEmpForm(){
  ...
  if(show){
    ...
    const isEdit=btn&&/Modifier/i.test(btn.textContent||'');
    if(!isEdit){
      editingEmployeId=null;
      editingEmployeHadBenai=false;
    ...
    resetEmpBenaiProvisionUi();
    syncEmpFormBenaiSection();
    requestAnimationFrame(()=>{try{w.scrollIntoView({behavior:'smooth',block:'nearest'});}catch(_){}});
  }
}
...
async function addEmploye(){
  ...
  if(!prenom||!nom){await benaiAlert('Prénom et Nom sont obligatoires');return;}
  const wantBen=!!document.getElementById('emp-opt-benai')?.checked;
  if(wantBen&&canManageBenaiUsersAdmin()){
    const em=(emailPro||email||'').trim().toLowerCase();
    if(!em||!em.includes('@')){
      await benaiAlert('Pour créer l’accès BenAI, renseignez au moins un e-mail valide (pro ou perso).');
      return;
    }
  }
  const newId=Date.now();
  const ann=getAnnuaire();
  ann.push({id:newId,prenom,nom,email,emailPro,tel,naissance,fonction,societe:soc,sync_ts:Date.now(),_deleted:false});
  saveAnnuaire(ann);
  let benaiMsg='';
  if(wantBen&&canManageBenaiUsersAdmin()){
    const fresh=ann.find(e=>e.id===newId);
    const role=String(document.getElementById('emp-benai-role')?.value||'assistante').trim();
    const pwd=(document.getElementById('emp-benai-pwd')?.value||'').trim();
    const customUid=(document.getElementById('emp-benai-app-uid')?.value||'').trim();
    const res=await createBenaiAccessFromAnnuaireEntry(fresh,{role,password:pwd,customAppUid:customUid});
    if(!res.ok){
      await benaiAlert('Fiche enregistrée, mais l’accès BenAI n’a pas pu être créé :\n\n'+res.error);
    }else{
      benaiMsg=res.remoteOk?' · accès BenAI créé.':' · accès BenAI créé localement (cloud en file).';
      renderUsersList();renderPwdList();
      logActivity(`Benjamin a créé l'accès BenAI (${res.roleLabel}) pour ${prenom} ${nom}`);
    }
  }
  ['emp-prenom','emp-nom','emp-email','emp-email-pro','emp-tel','emp-naissance'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
  resetEmpBenaiProvisionUi();
  syncEmpFormBenaiSection();
  const ok=document.getElementById('emp-ok');ok.textContent='✅ Enregistré'+benaiMsg+' !';
  setTimeout(()=>{ok.textContent='';toggleEmpForm();},wantBen?2200:1200);
...
async function deleteEmploye(id){
  ...
  let confirmMsg=`Retirer ${label} de l’annuaire ?`;
  if(linked){
    confirmMsg+=`\n\nUn accès BenAI actif (${linked.name||linked.id}) sera supprimé (cloud + local).`;
  }
  if(!await benaiConfirm(confirmMsg))return;
  if(linked){
    if(!canManageBenaiUsersAdmin()){
      await benaiAlert('La suppression d’accès BenAI est réservée à l’administrateur. Annulation : l’annuaire n’a pas été modifié.');
      return;
    }
...
function editEmploye(id){
  ...
  editingEmployeId=id;
  editingEmployeHadBenai=!!findBenaiAccountLinkedToAnnuaireEmploye(e);
...
async function saveEditEmploye(id){
  const prenom=document.getElementById('emp-prenom').value.trim();
  const nom=document.getElementById('emp-nom').value.trim();
  if(!prenom||!nom){await benaiAlert('Prénom et Nom sont obligatoires');return;}
  const wantBen=!!document.getElementById('emp-opt-benai')?.checked;
  if(wantBen&&!editingEmployeHadBenai&&canManageBenaiUsersAdmin()){
    const em=(document.getElementById('emp-email-pro')?.value||document.getElementById('emp-email')?.value||'').trim().toLowerCase();
    if(!em||!em.includes('@')){
      await benaiAlert('Pour créer l’accès BenAI, renseignez au moins un e-mail valide (pro ou perso).');
      return;
    }
  ...
  if(wantBen&&!editingEmployeHadBenai&&canManageBenaiUsersAdmin()){
    ...
    const pwd=(document.getElementById('emp-benai-pwd')?.value||'').trim();
    const customUid=(document.getElementById('emp-benai-app-uid')?.value||'').trim();
    const res=await createBenaiAccessFromAnnuaireEntry(ann[idx],{role,password:pwd,customAppUid:customUid});
    if(!res.ok)await benaiAlert('Fiche enregistrée, mais l’accès BenAI n’a pas pu être créé :\n\n'+res.error);
    else{
      benaiNote=' Accès BenAI créé.';
      renderUsersList();renderPwdList();
      logActivity(`Benjamin a créé l'accès BenAI (${res.roleLabel}) pour ${prenom} ${nom}`);
    }
  }
  ...
  const ok=document.getElementById('emp-ok');ok.textContent='✅ Modifié'+benaiNote+' !';
...
async function downloadPlanningAbsences(societe){
  if(currentUser?.role!=='admin')return;
  const mem=getMem();
  ...
  const fname=`Planning_absences_${societe}_${new Date().toISOString().slice(0,10)}.html`;
  ...
  if(!opened||opened.closed){
    try{
      ...
      a.click();
      a.remove();
      showDriveNotif('📥 Fichier HTML téléchargé — ouvrez-le puis Ctrl+P (A4 paysage) pour imprimer ou PDF');
    }catch(_){
      await benaiAlert('Impossible d’ouvrir une nouvelle fenêtre (bloqueur). Autorisez les pop-ups pour BenAI ou utilisez le fichier téléchargé.');
    }
  }
  setTimeout(()=>{try{URL.revokeObjectURL(url);}catch(_){}},opened&&!opened.closed?180000:90000);
  logActivity(`Benjamin a exporté le planning des absences ${label}`);
}
// ══════════════════════════════════════════
...
async function processFichesPaie(input){
  ...
  input.value='';
  const apiKey=await getApiKeyForChat();if(!apiKey){await benaiAlert('Clé API manquante');return;}
  paieResults=[];
  ...
  if(autoReady.length){
    const ok=await benaiConfirm(`✅ ${autoReady.length} fiche(s) ont un email perso trouvé.\n\nOuvrir automatiquement les brouillons Outlook maintenant ?`);
...
function askEmailPaie(idx){
  const r=paieResults[idx];if(!r)return;
  const label=`${r.prenom||''} ${r.nom||''}`.trim()||r.file||'';
  document.getElementById('paie-email-overlay')?.remove();
  const wrap=document.createElement('div');
  wrap.id='paie-email-overlay';
  wrap.className='lead-modal open';
  wrap.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:4500;display:flex;align-items:center;justify-content:center;padding:16px';
  wrap.innerHTML=`
    <div class="lead-modal-box" style="max-width:420px" role="dialog" aria-modal="true" aria-labelledby="paie-email-title" onclick="event.stopPropagation()">
      <div class="lead-modal-header">
        <div>
          <div id="paie-email-title" style="font-size:16px;font-weight:700">Email pour l’envoi</div>
          <div style="font-size:11px;color:var(--t3);margin-top:4px">${esc(label)}</div>
        </div>
        <button type="button" id="paie-email-close-x" style="background:transparent;border:none;color:var(--t2);font-size:20px;line-height:1;cursor:pointer;padding:4px 8px" aria-label="Fermer">×</button>
      </div>
      <div class="lead-modal-body">
        <div class="form-group">
          <div class="form-label">Adresse e-mail</div>
          <input type="email" class="form-input" id="paie-email-input" placeholder="prenom.nom@exemple.fr" style="font-family:inherit">
        </div>
        <div id="paie-email-inline-err" style="font-size:12px;color:var(--r);min-height:18px"></div>
        <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:8px;flex-wrap:wrap">
          <button type="button" id="paie-email-cancel" style="padding:10px 16px;background:var(--s3);color:var(--t1);border:1px solid var(--b1);border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit">Annuler</button>
          <button type="button" id="paie-email-save" style="padding:10px 16px;background:linear-gradient(135deg,var(--a),var(--a2));color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">Enregistrer</button>
        </div>
      </div>
    </div>`;
  const close=()=>{
  ...
  document.addEventListener('keydown',onKey);
  wrap.addEventListener('click',e=>{if(e.target===wrap)close();});
  const errEl=wrap.querySelector('#paie-email-inline-err');
  wrap.querySelector('#paie-email-close-x')?.addEventListener('click',close);
  wrap.querySelector('#paie-email-cancel')?.addEventListener('click',close);
  wrap.querySelector('#paie-email-save')?.addEventListener('click',()=>{
    const email=(document.getElementById('paie-email-input')?.value||'').trim().toLowerCase();
    if(!email||!email.includes('@')){if(errEl)errEl.textContent='Adresse e-mail invalide.';return;}
    if(errEl)errEl.textContent='';
  paieResults[idx].email=email;renderPaieList();
    close();
  });
  document.body.appendChild(wrap);
  const inp=document.getElementById('paie-email-input');
  if(inp){inp.focus();}
}
...
const HABIT_AI_LAST_REPORT_KEY='benai_habit_ai_last_report';
...
function buildUsageHabitContextText(){
  ...
  const topPagesOnly=top.map(([p])=>p);
  const lines=[
    `Utilisateur: ${currentUser?.name||'?'} (rôle: ${currentUser?.role||'?'})`,
    `Zones de l’app les plus consultées (ordre relatif sur cet appareil — sans volumes ni chiffres d’activité): ${topPagesOnly.length?topPagesOnly.join(', '):'aucune navigation enregistrée pour l’instant.'}`,
    `Aucun volume (SAV, CRM, compteurs) n’est transmis à l’IA — uniquement les noms de pages ci-dessus et le rôle.`
  ];
...
function hydrateHabitAiPanel(){
  ...
  hydrateHabitAiReportFromStorage();
}

function habitAiEscHtml(s){
  return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
}

/** Affichage HTML sûr à partir du markdown simple (##, listes -). */
function formatHabitAiReportHtml(raw){
  ...
  lines.forEach(line=>{
    ...
    if(/^##\s+/.test(trimmed)){
      closeUl();
      html+='<div class="habit-ai-sec">'+habitAiEscHtml(trimmed.replace(/^##\s+/,''))+'</div>';
    }else if(/^###\s+/.test(trimmed)){
      closeUl();
      html+='<div class="habit-ai-kv"><strong>'+habitAiEscHtml(trimmed.replace(/^###\s+/,''))+'</strong></div>';
    }else if(/^[-*]\s+/.test(trimmed)){
      if(!inUl){html+='<ul class="habit-ai-ul">';inUl=true;}
      html+='<li>'+habitAiEscHtml(trimmed.replace(/^[-*]\s+/,''))+'</li>';
    }else if(!trimmed){
      closeUl();
    }else{
      closeUl();
      html+='<p class="habit-ai-p">'+habitAiEscHtml(tEnd)+'</p>';
    }
  });
  closeUl();
  return html||'<p class="habit-ai-p" style="color:var(--t3)">(Réponse vide)</p>';
}

function setHabitAiReportVisible(rawText){
  const outEl=document.getElementById('habit-ai-output');
  const bar=document.getElementById('habit-ai-report-toolbar');
  ...
  outEl.innerHTML=formatHabitAiReportHtml(t);
  if(bar)bar.classList.add('is-visible');
}

function hydrateHabitAiReportFromStorage(){
  try{
    const raw=appStorage.getItem(HABIT_AI_LAST_REPORT_KEY)||'';
    setHabitAiReportVisible(raw);
  }catch{
    setHabitAiReportVisible('');
  }
}

function copyHabitAiLastReport(){
  const raw=appStorage.getItem(HABIT_AI_LAST_REPORT_KEY)||'';
  if(!String(raw).trim()){showDriveNotif('Aucun rapport à copier — lancez d’abord une analyse.');return;}
  const run=()=>showDriveNotif('Rapport copié — collez-le dans Cursor ou votre suivi.');
  if(navigator.clipboard&&navigator.clipboard.writeText){
    navigator.clipboard.writeText(raw).then(run).catch(()=>{
      try{
        const ta=document.createElement('textarea');
        ta.value=raw;ta.style.position='fixed';ta.style.left='-9999px';
        document.body.appendChild(ta);ta.select();document.execCommand('copy');
        document.body.removeChild(ta);run();
      }catch{showDriveNotif('Copie impossible');}
    });
  }else{
    try{
      const ta=document.createElement('textarea');
...
function clearHabitAiReport(){
  appStorage.removeItem(HABIT_AI_LAST_REPORT_KEY);
  setHabitAiReportVisible('');
  showDriveNotif('Rapport effacé de ce navigateur.');
}
...
async function runHabitImprovementSuggestions(){
  ...
  const userContent=`Voici un résumé factuel d’utilisation de BenAI (sans chiffres métier).\n\nTa sortie sera RELUE par un développeur humain ou une IA de code : elle doit être ACTIONNABLE.\n\nRègles strictes :\n- Réponds en français.\n- Propose EXACTEMENT 5 à 7 améliorations numérotées.\n- Pour CHAQUE amélioration, utilise EXACTEMENT ce gabarit (lignes telles quelles, avec les libellés) :\n\n## N — Titre court (max 8 mots)\n- **Module** : un parmi [benai, messages, sav, leads, notes, absences, annuaire, paie, admin, bugs, evolution, guide, autre]\n- **Problème / opportunité** : une phrase claire.\n- **Comportement attendu** : 1 à 2 phrases (ce que l’app doit faire).\n- **Critères d’acceptation** : 2 ou 3 puces commençant par « - » testables par un testeur.\n- **Priorité** : P1 | P2 | P3 (P1 = le plus urgent)\n- **Dépendances / risques** : une phrase (ou « Aucun »).\n\nPas d’introduction ni de conclusion hors de ces blocs. Pas de markdown gras ailleurs que les libellés **...** ci-dessus.\n\n---\n${snap}`;
  const systemAdd=`Tu es BenAI côté évolution produit. Tu rédiges des fiches « backlog » lisibles par un développeur. Tu ne promets pas de délais. Aucune action automatique dans l’app. Zéro jargon vide : chaque phrase doit aider à coder ou à tester.`;
  ...
  try{
    ...
    trackTokens(currentUser.id,inp,outTok);
    const costStr=estimateCost(inp,outTok);
    ...
    if(met)met.textContent=`Dernier appel : ${latency} ms · ${inp} + ${outTok} tokens · ~${costStr} € (estim. Sonnet — indicatif)`;
    const rawText=(data.content&&data.content[0]&&data.content[0].text)?String(data.content[0].text):'';
    try{appStorage.setItem(HABIT_AI_LAST_REPORT_KEY,rawText);}catch(_){}
    setHabitAiReportVisible(rawText);
    try{logActivity(`${currentUser.name} a lancé l’analyse « suggestions usage » IA`);}catch{}
  }catch(e){
    const aborted=e?.name==='AbortError'||/aborted|AbortError/i.test(String(e?.message||''));
    const errTxt=aborted?'Délai dépassé (réponse IA trop longue ou réseau lent). Réessayez dans un instant.':(e?.message||e||'?');
    if(met)met.textContent='Erreur : '+errTxt;
    showDriveNotif(aborted?'⏱️ Analyse interrompue — délai réseau. Réessayez.':'❌ Analyse IA impossible — vérifiez la clé API ou le réseau.');
  }finally{
...
async function validateEvol(id){
  const e=EVOLUTIONS.find(x=>x.id===id);
  if(e)await benaiAlert(`✅ Noté ! "${e.title}" sera développé dans la prochaine mise à jour BenAI.`);
  dismissEvol(id);
}
...
function getRoleGuideAckLocalStorageKey(role){
  return ROLE_GUIDE_ACK_LOCAL_PREFIX+getRoleGuideAckKey(role||'assistante');
}
function hasAcknowledgedRoleGuide(role){
  if(!currentUser||currentUser.id==='benjamin')return true;
  const key=getRoleGuideAckKey(role||currentUser.role);
  if(appStorage.getItem(key)==='1')return true;
  try{
    const lk=getRoleGuideAckLocalStorageKey(role||currentUser.role);
...
function acknowledgeRoleGuide(){
  if(!currentUser)return;
  const key=getRoleGuideAckKey(currentUser.role);
  appStorage.setItem(key,'1');
  try{localStorage.setItem(getRoleGuideAckLocalStorageKey(currentUser.role),'1');}catch(e){}
  renderGuidePage();
...
function renderGuidePage(){
  ...
  const pilotCompanyTitle=getBenaiPilotageCompanyTitle();
  ...
  const roleGuideDirecteurCo=[
    ...
    'Exports : menu secteur sur le dashboard + bouton « Performance secteurs » ; paquets « Télécharger tous les tableaux CRM » et export leads. Les fichiers respectent les leads et secteurs visibles pour toi.',
    'Notifications : leads à attribuer, alertes métier, rappels devis selon les dossiers visibles ; le badge sur Leads suit surtout les dossiers à traiter sur ton périmètre.',
    'Messages : échanges rapides avec l’équipe. Absences : tu peux consulter le planning si ton équipe le renseigne. Signalement : menu « Signaler » (page, étapes, capture si possible).'
  ];
  ...
  const roleGuide={
    admin:[
      ...
      'Tu peux jeter un œil régulièrement aux tickets (admin) et aux connexions.'
    ],
    directeur_co:roleGuideDirecteurCo,
    directeur_general:roleGuideDirecteurGeneral,
    commercial:[
      ...
      'Vendu : le prix vendu HT exact du dossier est demandé pour enregistrer la vente. Perdu : un motif détaillé est demandé pour clôturer — ces champs permettent d’enregistrer la fiche.',
      'Hors zone géographique habituelle : remplis la justification « hors secteur » pour pouvoir enregistrer.',
      'Badge et notifications : dossiers en alerte et rappels à l’heure — tu peux réagir ou ajuster la date dans la fiche.',
      'Messages : relances, avis ou infos client ; complète la fiche lead pour tout ce qui engage le dossier (statut, montants, dates).',
      'Absences : tu peux déclarer les tiennes pour que l’équipe puisse anticiper si besoin.',
...
function refreshAdmin(){
  ...
  const mem=getMem();
  const savs=mem.sav||[];
  const savOpen=savs.filter(s=>!s._deleted&&!s.archive&&(s.statut==='nouveau'||s.statut==='en_cours')).length;
  const savKpi=document.getElementById('admin-sav');if(savKpi)savKpi.textContent=String(savOpen);
  ...
  if(actList){
    ...
    actList.innerHTML=mem.activity.slice(-10).reverse().map(a=>`<div class="act-item"><div class="act-av" style="background:${getUserColor(a.user)}">${getUserInitial(a.user)}</div><div class="act-txt">${esc(a.txt)}</div><div class="act-time">${a.time}</div></div>`).join('');
      ...
      actList.innerHTML='<div style="color:var(--t3);font-size:12px;padding:8px 0">Aucune activité récente</div>';
    }
  }
...
async function modifierBenaiLoginUid(uid){
  if(!canAssignBenaiLoginPseudo()){await benaiAlert('Seul Benjamin peut modifier l’identifiant de connexion.');return;}
  const u=findUserById(uid)||findUserById(normalizeId(uid));
  if(!u)return;
  const isSelfBenjamin=normalizeId(uid)==='benjamin';
  if(isSelfBenjamin){
    if(!await benaiConfirm('Tu définis le pseudo pour te connecter à BenAI (Supabase). Ton compte dans l’app reste « Benjamin » ; seul le mot de passe / email côté Supabase compte pour la connexion. Continuer ?'))return;
  }
  ...
  if(!isLikelyUuid(authRaw)){
    await benaiAlert('Compte sans identifiant Supabase (auth). Connecte-toi avec email + mot de passe Supabase une fois pour lier la session, puis réessaie.');
    return;
  }
  const actuel=String(u.id||'').trim();
  document.getElementById('benai-login-edit-overlay')?.remove();
  const wrap=document.createElement('div');
  wrap.id='benai-login-edit-overlay';
  wrap.className='lead-modal open';
  wrap.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:4500;display:flex;align-items:center;justify-content:center;padding:16px';
  const hintSelf=isSelfBenjamin?`Identifiant interne BenAI : <strong>${esc(actuel)}</strong> (inchangé dans l’app).`:`Actuel dans BenAI : <strong>${esc(actuel)}</strong>.`;
  wrap.innerHTML=`
    <div class="lead-modal-box" style="max-width:460px" role="dialog" aria-modal="true" aria-labelledby="benai-login-edit-title" onclick="event.stopPropagation()">
      <div class="lead-modal-header">
        <div>
          <div id="benai-login-edit-title" style="font-size:16px;font-weight:700">Identifiant de connexion</div>
          <div style="font-size:11px;color:var(--t3);margin-top:6px;line-height:1.45">${esc(u.name)} — ${hintSelf}<br>Lettres, chiffres et _ (accents et espaces seront normalisés).</div>
        </div>
        <button type="button" id="benai-login-close-x" style="background:transparent;border:none;color:var(--t2);font-size:20px;line-height:1;cursor:pointer;padding:4px 8px" aria-label="Fermer">×</button>
      </div>
      <div class="lead-modal-body">
        <div class="form-group">
          <div class="form-label">Nouvel identifiant</div>
          <input type="text" class="form-input" id="benai-login-new" value="${esc(actuel)}" autocomplete="username" style="font-family:monospace;font-size:13px">
        </div>
        <div id="benai-login-inline-err" style="font-size:12px;color:var(--r);min-height:18px"></div>
        <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:8px;flex-wrap:wrap">
          <button type="button" id="benai-login-cancel" style="padding:10px 16px;background:var(--s3);color:var(--t1);border:1px solid var(--b1);border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit">Annuler</button>
          <button type="button" id="benai-login-save" style="padding:10px 16px;background:linear-gradient(135deg,var(--a),var(--a2));color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">Enregistrer</button>
        </div>
      </div>
    </div>`;
  const close=()=>{
  ...
  document.addEventListener('keydown',onKey);
  wrap.addEventListener('click',e=>{if(e.target===wrap)close();});
  wrap.querySelector('#benai-login-close-x')?.addEventListener('click',close);
  wrap.querySelector('#benai-login-cancel')?.addEventListener('click',close);
  const errEl=wrap.querySelector('#benai-login-inline-err');
  wrap.querySelector('#benai-login-save')?.addEventListener('click',()=>{
    const np=(document.getElementById('benai-login-new')?.value||'').trim();
    if(!np){if(errEl)errEl.textContent='Saisissez un identifiant.';return;}
    const candidate=proposedLoginUidFromPseudoField(np,actuel);
    if(!candidate||candidate==='user'){if(errEl)errEl.textContent='Identifiant invalide.';return;}
    const reserved=new Set(['benjamin','benai','admin']);
    if(reserved.has(candidate)){if(errEl)errEl.textContent='Identifiant réservé.';return;}
    if(normalizeId(candidate)===normalizeId(actuel)){if(errEl)errEl.textContent='Identique à l’actuel.';return;}
    if(getAllUsers().some(x=>normalizeId(x.id)===normalizeId(candidate)&&normalizeId(x.id)!==normalizeId(actuel))){
      if(errEl)errEl.textContent='Cet identifiant est déjà utilisé.';
      return;
    }
    if(errEl)errEl.textContent='';
    void (async()=>{
      ...
      if(!cloud.ok){await benaiAlert('Échec serveur : '+(cloud.error||'?'));return;}
      ...
      close();
      await benaiAlert(`Identifiant de connexion mis à jour : ${resolved}`);
    })();
  });
  document.body.appendChild(wrap);
  const inp=document.getElementById('benai-login-new');
  if(inp){inp.focus();try{inp.select();}catch(e){}}
}
...
function getExtraUserById(id){
  const n=normalizeId(String(id??''));
  if(!n)return null;
  return getExtraUsers().find(u=>normalizeId(String(u.id??''))===n)||null;
}
...
function getCommercialArchives(){
  try{return JSON.parse(appStorage.getItem('benai_commercial_archives'))||[];}catch(e){return[];}
}
function saveCommercialArchives(list){
  appStorage.setItem('benai_commercial_archives',JSON.stringify(Array.isArray(list)?list:[]));
  scheduleAppStoragePersist();
}
function getCommercialArchiveByUid(uid){
...
function archiveCommercialUserSnapshot(u){
  ...
  if(r!=='commercial'&&r!=='directeur_co')return;
  const id=String(u.id||'').trim();
  if(!id)return;
  const list=getCommercialArchives().filter(a=>normalizeId(a.id)!==normalizeId(id));
  list.push({
    id,
    ...
    societe:u.societe||'',
    vehicule:u.vehicule||'',
    color:u.color||'linear-gradient(135deg,#888,#555)',
    initial:String(u.initial||'?').slice(0,2),
    deletedAt:Date.now()
  });
  saveCommercialArchives(list);
}
...
function resolveCrmCommercialLabel(uid){
  ...
  if(arch){
    const nm=String(arch.name||arch.id||'').trim()||String(uid);
    return`${nm} (compte supprimé)`;
  }
  return String(uid);
}
...
// Trouver un utilisateur : id BenAI (slug), ou UUID auth Supabase, ou email
function findUserById(uid){
  const raw=String(uid??'').trim();
  if(!raw)return null;
  const normalized=normalizeId(raw);
  const all=getAllUsers();
  let hit=all.find(u=>normalizeId(u.id)===normalized);
  if(hit)return hit;
  const rawLc=raw.toLowerCase();
  if(isLikelyUuid(raw)){
    hit=all.find(u=>String(u.auth_uid||u.authUid||'').toLowerCase()===rawLc);
    if(hit)return hit;
  }
  hit=all.find(u=>String(u.email||'').toLowerCase()===rawLc);
  return hit||null;
}
...
function closeBenaiAccessModalOverlay(){
  document.getElementById('benai-access-modal-overlay')?.remove();
}

async function openBenaiAccessModalForAnnEntry(annEntry){
  if(!canManageBenaiUsersAdmin()){await benaiAlert('Réservé à l’administrateur.');return;}
  if(!annEntry){await benaiAlert('Contact introuvable.');return;}
  if(findBenaiAccountLinkedToAnnuaireEmploye(annEntry)){
    await benaiAlert('Un accès BenAI existe déjà pour ce salarié.');
    return;
  }
  closeBenaiAccessModalOverlay();
  const label=`${String(annEntry.prenom||'').trim()} ${String(annEntry.nom||'').trim()}`.trim();
  const showPseudo=canAssignBenaiLoginPseudo();
  const wrap=document.createElement('div');
  wrap.id='benai-access-modal-overlay';
  wrap.className='lead-modal open';
  wrap.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.72);z-index:4600;display:flex;align-items:center;justify-content:center;padding:16px';
  wrap.innerHTML=`
    <div class="lead-modal-box" style="max-width:440px;width:100%" role="dialog" aria-modal="true" onclick="event.stopPropagation()">
      <div class="lead-modal-header">
        <div>
          <div style="font-size:16px;font-weight:700">Créer l’accès BenAI</div>
          <div style="font-size:12px;color:var(--t3);margin-top:6px">${esc(label)}</div>
        </div>
        <button type="button" id="benai-acc-m-close" style="background:transparent;border:none;color:var(--t2);font-size:22px;cursor:pointer;padding:4px 8px" aria-label="Fermer">×</button>
      </div>
      <div class="lead-modal-body">
        <div class="form-group">
          <div class="form-label">Rôle</div>
          <select class="form-input" id="benai-acc-m-role">
            <option value="assistante">Assistante (BenAI complet)</option>
            <option value="commercial">Commercial (CRM)</option>
            <option value="directeur_co">Directeur commercial (CRM complet)</option>
            <option value="directeur_general">Directeur général</option>
          </select>
        </div>
        <div class="form-group">
          <div class="form-label">Mot de passe <span style="font-weight:400;color:var(--t3);font-size:10px">(auto)</span></div>
          <div style="display:flex;gap:6px;align-items:stretch">
            <input type="text" class="form-input" id="benai-acc-m-pwd" readonly style="flex:1;font-family:'JetBrains Mono',monospace;font-size:12px">
            <button type="button" class="equipe-link-btn" id="benai-acc-m-pwd-regen" style="flex-shrink:0;padding:8px 12px" title="Regénérer">↻</button>
          </div>
        </div>
        <div class="form-group" id="benai-acc-m-pseudo-wrap" style="display:${showPseudo?'block':'none'}">
          <div class="form-label">Identifiant de connexion <span style="font-size:10px;color:var(--t3)">(optionnel — Benjamin)</span></div>
          <input type="text" class="form-input" id="benai-acc-m-uid" placeholder="ex. vente_sud" autocomplete="off" spellcheck="false">
        </div>
        <div id="benai-acc-m-status" style="font-size:12px;min-height:18px;margin-top:6px"></div>
        <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:14px;flex-wrap:wrap">
          <button type="button" id="benai-acc-m-cancel" style="padding:9px 16px;background:transparent;border:1px solid var(--b2);color:var(--t2);border-radius:8px;cursor:pointer;font-family:inherit;font-size:13px">Annuler</button>
          <button type="button" id="benai-acc-m-submit" class="btn-primary" style="padding:9px 18px;font-size:13px">Créer l’accès</button>
        </div>
      </div>
    </div>`;
  const pwdEl=()=>wrap.querySelector('#benai-acc-m-pwd');
  pwdEl().value=generateBenAIPassword(14);
  const close=()=>{
  ...
  wrap.addEventListener('click',e=>{if(e.target===wrap)close();});
  wrap.querySelector('#benai-acc-m-close')?.addEventListener('click',close);
  wrap.querySelector('#benai-acc-m-cancel')?.addEventListener('click',close);
  wrap.querySelector('#benai-acc-m-pwd-regen')?.addEventListener('click',()=>{pwdEl().value=generateBenAIPassword(14);});
  wrap.querySelector('#benai-acc-m-submit')?.addEventListener('click',async()=>{
    const statusEl=wrap.querySelector('#benai-acc-m-status');
    const role=String(wrap.querySelector('#benai-acc-m-role')?.value||'assistante').trim();
    const pwd=String(pwdEl().value||'').trim();
    const customUid=String(wrap.querySelector('#benai-acc-m-uid')?.value||'').trim();
    if(statusEl){
      statusEl.style.color='var(--a)';
      statusEl.textContent='⏳ Création en cours…';
    }
    const res=await createBenaiAccessFromAnnuaireEntry(annEntry,{role,password:pwd,customAppUid:customUid});
    if(!res.ok){
      if(statusEl){statusEl.style.color='var(--r)';statusEl.textContent='⚠️ '+res.error;}
      return;
    }
    showDriveNotif(`✅ Accès ${res.roleLabel} · ${label} — ${res.uid}`);
    logActivity(`Benjamin a créé l'accès BenAI (${res.roleLabel}) pour ${label}`);
    renderUsersList();renderPwdList();renderAnnuaire();
    const detail=res.remoteOk
      ?`Identifiant : ${res.uid}\nE-mail : ${res.email}\nMot de passe (copiez-le) :\n${res.pwd}`
      :`Compte créé localement.\nIdentifiant : ${res.uid}\nMot de passe :\n${res.pwd}\n\nSynchronisation cloud en attente.`;
    await benaiAlert(detail);
    close();
  });
  document.body.appendChild(wrap);
}

async function showAddUser(preselectEmpId){
  if(!canManageBenaiUsersAdmin()){await benaiAlert('Réservé à l’administrateur.');return;}
  const id=preselectEmpId!=null&&preselectEmpId!==undefined?String(preselectEmpId).trim():'';
  if(!id){
    await benaiAlert('Créez d’abord la fiche dans l’annuaire, puis utilisez « + Accès BenAI » sur la carte (ou cochez l’option lors de l’enregistrement).');
    return;
  }
  const emp=(getAnnuaireActive()||[]).find(e=>String(e.id)===id);
  if(!emp){await benaiAlert('Contact introuvable.');return;}
  await openBenaiAccessModalForAnnEntry(emp);
}

async function openBenaiAccessForEmployeCard(id){
  await showAddUser(id);
}

/** Crée un compte BenAI à partir d’une fiche annuaire (employé + accès unifiés). */
async function createBenaiAccessFromAnnuaireEntry(annEntry,opts){
  opts=opts||{};
  const fail=msg=>Promise.resolve({ok:false,error:String(msg||'')});
  if(!annEntry)return fail('Fiche contact introuvable.');
  if(findBenaiAccountLinkedToAnnuaireEmploye(annEntry))return fail('Un accès BenAI existe déjà pour ce salarié (même nom ou même e-mail).');
  const role=String(opts.role||'assistante').trim();
  let pwd=String(opts.password||'').trim();
  if(!pwd||pwd.length<10)pwd=generateBenAIPassword(14);
  const prenom=String(annEntry.prenom||'').trim();
  const nom=String(annEntry.nom||'').trim();
  const soc=annEntry.societe||'nemausus';
  const fonction=String(annEntry.fonction||'Autre').trim()||'Autre';
  ...
  if(canAssignBenaiLoginPseudo()){
    const custom=String(opts.customAppUid||'').trim();
    if(custom){
      uid=proposedLoginUidFromPseudoField(custom,uid);
      const reserved=new Set(['benjamin','benai','admin']);
      if(reserved.has(uid)||uid==='user')return fail('Identifiant de connexion réservé ou invalide.');
      if(getAllUsers().some(u=>normalizeId(u.id)===normalizeId(uid)))return fail('Cet identifiant est déjà utilisé.');
    }
  }
  const email=(annEntry.emailPro||annEntry.email||'').trim().toLowerCase();
  if(!email||!email.includes('@'))return fail('Email introuvable (remplissez email perso ou pro sur la fiche).');
  if(role==='directeur_co'){
    const existingDco=getAllUsers().filter(u=>u.role==='directeur_co');
    const conflict=existingDco.some(u=>{
      const s=u.societe||'nemausus';
      if(soc==='les-deux'||s==='les-deux')return true;
      return s===soc;
    });
    if(conflict)return fail('Un directeur commercial existe déjà pour cette société (ou un compte « les deux » couvre les deux). Supprimez ou modifiez l’existant avant d’en créer un autre.');
  }
  const extras=getExtraUsers();
  ...
  if(!createResult.ok&&!useLocalFallback){
    ...
    return{ok:false,error:errShown};
  }
  if(useLocalFallback){
  ...
  return{ok:true,pwd,uid,email,roleLabel,remoteOk:!!createResult.ok};
}

async function supprimerUtilisateur(uid, opts){
  opts=opts||{};
  if(!canManageBenaiUsersAdmin()){await benaiAlert('Réservé à l’administrateur.');return;}
  if(normalizeId(uid)==='benjamin'){await benaiAlert('Benjamin ne peut pas être supprimé.');return;}
  const u=findUserById(uid)||findUserById(normalizeId(uid));
  if(!u){await benaiAlert('Utilisateur introuvable.');return;}
  const effectiveId=u.id;
  const roleStr=String(u.role||'');
  const archiveCrmProfile=roleStr==='commercial'||roleStr==='directeur_co';
  const confirmMsg=archiveCrmProfile
    ?`Supprimer ${u.name||effectiveId} définitivement ?\n\nSon compte et ses accès seront retirés, mais les leads et objectifs CRM déjà associés à son identifiant resteront pour l’historique et les exports (nom affiché : « … (compte supprimé) »).`
...
function editUser(uid){
  ...
  const showVeh=u.role==='commercial'||u.role==='directeur_co'||u.role==='directeur_general';
  let vehicule=u.vehicule||'';
  document.getElementById('user-edit-overlay')?.remove();
  const wrap=document.createElement('div');
  wrap.id='user-edit-overlay';
  wrap.className='lead-modal open';
  wrap.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:4500;display:flex;align-items:center;justify-content:center;padding:16px';
  wrap.innerHTML=`
    <div class="lead-modal-box" style="max-width:440px" role="dialog" aria-modal="true" aria-labelledby="user-edit-title" onclick="event.stopPropagation()">
      <div class="lead-modal-header">
        <div>
          <div id="user-edit-title" style="font-size:16px;font-weight:700">Modifier l’utilisateur</div>
          <div style="font-size:11px;color:var(--t3);margin-top:4px">${esc(u.name)} <span style="opacity:.85">(${esc(uid)})</span></div>
        </div>
        <button type="button" id="user-edit-close-x" style="background:transparent;border:none;color:var(--t2);font-size:20px;line-height:1;cursor:pointer;padding:4px 8px" aria-label="Fermer">×</button>
      </div>
      <div class="lead-modal-body">
        <div class="lead-modal-section">
          <div class="form-group">
            <div class="form-label">Nom affiché</div>
            <input type="text" class="form-input" id="user-edit-name" value="${esc(u.name)}" style="font-family:inherit">
          </div>
        </div>
        <div class="lead-modal-section" id="user-edit-veh-wrap" style="display:${showVeh?'block':'none'}">
          <div class="form-group">
            <div class="form-label">Véhicule de société (optionnel)</div>
            <input type="text" class="form-input" id="user-edit-vehicule" value="${esc(vehicule)}" placeholder="ex. AB-123-CD" style="font-family:inherit">
          </div>
        </div>
        <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:8px;flex-wrap:wrap">
          <button type="button" id="user-edit-cancel" style="padding:10px 16px;background:var(--s3);color:var(--t1);border:1px solid var(--b1);border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit">Annuler</button>
          <button type="button" id="user-edit-save" style="padding:10px 16px;background:linear-gradient(135deg,var(--a),var(--a2));color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">Enregistrer</button>
        </div>
      </div>
    </div>`;
  const close=()=>{
  ...
  document.addEventListener('keydown',onKey);
  wrap.addEventListener('click',e=>{if(e.target===wrap)close();});
  wrap.querySelector('#user-edit-close-x')?.addEventListener('click',close);
  wrap.querySelector('#user-edit-cancel')?.addEventListener('click',close);
  wrap.querySelector('#user-edit-save')?.addEventListener('click',()=>{
    const nouveau=(document.getElementById('user-edit-name')?.value||'').trim();
    if(!nouveau){void benaiAlert('Le nom affiché ne peut pas être vide.');return;}
    if(showVeh)vehicule=(document.getElementById('user-edit-vehicule')?.value||'').trim();
  if(USERS[uid]){
      USERS[uid].name=nouveau;
    USERS[uid].vehicule=vehicule;
  }
  const extras=getExtraUsers();
  const idx=extras.findIndex(e=>e.id===uid);
    if(idx>-1){extras[idx].name=nouveau;extras[idx].vehicule=vehicule;saveExtraUsers(extras);}
  const overrides=JSON.parse(appStorage.getItem('benai_name_overrides')||'{}');
    overrides[uid]=nouveau;
  appStorage.setItem('benai_name_overrides',JSON.stringify(overrides));
  renderUsersList();renderPwdList();
    logActivity(`Benjamin a modifié le nom de ${uid} → ${nouveau}`);
    close();
    void benaiAlert(`✅ Nom modifié : ${nouveau}`);
  });
  document.body.appendChild(wrap);
  const inp=document.getElementById('user-edit-name');
  if(inp){inp.focus();try{inp.select();}catch(e){}}
}
...
function changerMdp(uid,name){
  if(currentUser?.role!=='admin'){void benaiAlert('⚠️ Réservé à l’administrateur');return;}
  document.getElementById('pwd-change-overlay')?.remove();
  const wrap=document.createElement('div');
  wrap.id='pwd-change-overlay';
  wrap.className='lead-modal open';
  wrap.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:4500;display:flex;align-items:center;justify-content:center;padding:16px';
  wrap.innerHTML=`
    <div class="lead-modal-box" style="max-width:420px" role="dialog" aria-modal="true" aria-labelledby="pwd-change-title" onclick="event.stopPropagation()">
      <div class="lead-modal-header">
        <div>
          <div id="pwd-change-title" style="font-size:16px;font-weight:700">Mot de passe — ${esc(name)}</div>
          <div style="font-size:11px;color:var(--t3);margin-top:6px;line-height:1.45">Minimum 6 caractères · au moins 1 majuscule · au moins 1 caractère spécial (!@#$%…)</div>
        </div>
        <button type="button" id="pwd-change-close-x" style="background:transparent;border:none;color:var(--t2);font-size:20px;line-height:1;cursor:pointer;padding:4px 8px" aria-label="Fermer">×</button>
      </div>
      <div class="lead-modal-body">
        <div class="form-group">
          <div class="form-label">Nouveau mot de passe</div>
          <input type="password" class="form-input" id="pwd-change-p1" autocomplete="new-password" style="font-family:inherit">
        </div>
        <div class="form-group">
          <div class="form-label">Confirmer</div>
          <input type="password" class="form-input" id="pwd-change-p2" autocomplete="new-password" style="font-family:inherit">
        </div>
        <div id="pwd-change-inline-err" style="font-size:12px;color:var(--r);min-height:18px;margin-bottom:4px"></div>
        <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:8px;flex-wrap:wrap">
          <button type="button" id="pwd-change-cancel" style="padding:10px 16px;background:var(--s3);color:var(--t1);border:1px solid var(--b1);border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit">Annuler</button>
          <button type="button" id="pwd-change-save" style="padding:10px 16px;background:linear-gradient(135deg,var(--a),var(--a2));color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">Enregistrer</button>
        </div>
      </div>
    </div>`;
  const close=()=>{
  ...
  document.addEventListener('keydown',onKey);
  wrap.addEventListener('click',e=>{if(e.target===wrap)close();});
  wrap.querySelector('#pwd-change-close-x')?.addEventListener('click',close);
  wrap.querySelector('#pwd-change-cancel')?.addEventListener('click',close);
  const errEl=wrap.querySelector('#pwd-change-inline-err');
  wrap.querySelector('#pwd-change-save')?.addEventListener('click',()=>{
    const nouveau=(document.getElementById('pwd-change-p1')?.value||'').trim();
    const p2=(document.getElementById('pwd-change-p2')?.value||'').trim();
    if(!nouveau){if(errEl)errEl.textContent='Saisissez un mot de passe.';return;}
    if(nouveau!==p2){if(errEl)errEl.textContent='Les deux champs ne correspondent pas.';return;}
    const vErr=validatePassword(nouveau);
    ...
    void (async()=>{
      ...
      if(cloud.ok){
        await benaiAlert(`✅ Mot de passe de ${name} modifié (BenAI + cloud).`);
      }else if(SUPABASE_CONFIG.enabled){
        await benaiAlert(`✅ Mot de passe local enregistré pour ${name}.\n\n⚠️ Cloud : ${cloud.error||'non synchronisé'}\nDéployez la fonction Supabase « update-user-password » (même principe que delete-user), puis réessayez.`);
      }else{
        await benaiAlert(`✅ Mot de passe de ${name} modifié (stockage local).`);
      }
    })();
  });
  document.body.appendChild(wrap);
  document.getElementById('pwd-change-p1')?.focus();
}
...
const THEME_STORAGE_KEY='benai_theme';
/** « dark » uniquement si choisi explicitement ; tout le reste → clair (pas de suivi du thème système). */
function getStoredThemeChoice(){
  const v=String(appStorage.getItem(THEME_STORAGE_KEY)||'light').toLowerCase().trim();
  return v==='dark'?'dark':'light';
}
function applyTheme(){
  const mode=getStoredThemeChoice();
  document.body.classList.toggle('light',mode==='light');
  try{
    document.documentElement.style.setProperty('color-scheme',mode==='dark'?'dark':'light');
    const meta=document.querySelector('meta[name="theme-color"]');
    if(meta)meta.setAttribute('content',mode==='dark'?'#101010':'#E8943A');
  }catch(e){}
}
function setThemeChoice(mode){
  const normalized=String(mode||'').toLowerCase()==='dark'?'dark':'light';
  appStorage.setItem(THEME_STORAGE_KEY,normalized);
  scheduleAppStoragePersist();
  applyTheme();
...
function syncSettingsThemeButtons(){
  const mode=getStoredThemeChoice();
  const bL=document.getElementById('settings-theme-light');
  const bD=document.getElementById('settings-theme-dark');
  if(bL)bL.classList.toggle('active',mode==='light');
  if(bD)bD.classList.toggle('active',mode==='dark');
}
...
function importData(input){
  ...
  reader.onload=async e=>{
    try{
      ...
      if(!await benaiConfirm(`Restaurer la sauvegarde du ${new Date(backup.date).toLocaleDateString('fr-FR')} ?\n\nToutes les données actuelles seront remplacées.`))return;
      saveMem(backup.data);
      savePwds(backup.pwds||{});
      saveExtraUsers(backup.extras||[]);
      saveAccess(backup.access||{});
      if(backup.annuaire)saveAnnuaire(backup.annuaire);
      await benaiAlert('✅ Données restaurées ! La page va se recharger.');
      location.reload();
    }catch(err){await benaiAlert('❌ Erreur : fichier de sauvegarde invalide');}
  };
...
/** Seuil « polling mort » : au-delà, les navigateurs peuvent déjà ralentir les timers (onglet en arrière-plan) — évite les faux rapports auto-bug ~90s. */
const POLLING_STALE_MS=Math.max(240000,POLLING_INTERVAL_MS*15);
...
async function manualBackupToDrive(){
  const clientId=getGDriveClientId();
  if(!clientId){await benaiAlert('⚠️ Configure ton Client ID Google Drive dans les paramètres.');return;}
  if(!window.google?.accounts?.oauth2){await benaiAlert('⚠️ Google Identity Services non chargé. Vérifie ta connexion internet.');return;}
  if(!gdriveTokenClient)initGDriveClient();
  if(!gdriveTokenClient)return;
...
async function registerBenAISW(){
  ...
  try{
    const swCode=`
      const CACHE_NAME='benai-shell-v4';
...
function canViewLeadFullTimeline(){
  return currentUser?.role==='admin'||isCRMScopePilotageRole(currentUser?.role);
}
...
function crmAssigneeCoversLeadSociete(user,leadSoc){
  if(!user)return false;
  const s=String(user.societe||'').trim().toLowerCase();
  if(s==='les-deux')return true;
  if(s==='lambert')return leadSoc==='lambert';
...
function getLeadCreationYearValue(l){
  const d=new Date(l?.date_creation||0);
  if(isNaN(d.getTime()))return null;
  return d.getFullYear();
}
/** Hors vues actives du CRM : archive manuelle, ventes (vert) passées au mois suivant (auto), OU création strictement avant l’année civile en cours. */
function isLeadCrmArchivedView(l){
  if(!l)return false;
  if(l.archive)return true;
  const y=getLeadCreationYearValue(l);
  if(y===null)return false;
  return y<new Date().getFullYear();
}
/** Mois calendaire de référence pour archiver un vendu : date de signature si saisie, sinon date de création (aligné tableau des ventes). */
function getLeadVenduArchiveMonthAnchor(l){
  if(!l||l.statut!=='vert')return null;
  const ds=String(l.date_signature||'').trim();
  if(ds){
...
/** Dès qu’un nouveau mois civil commence : les vendus du mois précédent passent en archive (conservation des données, visibles filtre Archives + tableau des ventes). */
function autoArchiveVenduLeadsPastCurrentMonth(){
  const now=new Date();
  const curY=now.getFullYear(),curM=now.getMonth();
  const leads=getLeads();
  let changed=false;
  leads.forEach(l=>{
    if(!l||l._deleted||l.statut!=='vert'||l.archive)return;
    const am=getLeadVenduArchiveMonthAnchor(l);
    if(!am)return;
    if(am.year<curY||(am.year===curY&&am.month<curM)){
      l.archive=true;
      l.date_modification=new Date().toISOString();
      try{addLeadTimelineEntry(l,'Vendu — archivé automatiquement (mois écoulé)','BenAI');}catch(e){}
      changed=true;
    }
  });
  if(changed)saveLeads(leads,true);
}
/** Objectifs CRM par entreprise : CA HT de référence + montants optionnels par mois (YYYY-MM). */
function normalizeSocObjEntry(rawSoc,baseCaHT){
  const caHT=Math.max(0,Number(rawSoc?.caHT)||baseCaHT||0);
  const months={};
  if(rawSoc&&rawSoc.months&&typeof rawSoc.months==='object'&&!Array.isArray(rawSoc.months)){
    Object.keys(rawSoc.months).forEach(k=>{
      months[k]=Math.max(0,Number(rawSoc.months[k])||0);
    });
  }
  return{caHT,months};
}
function getCrmSocieteObjectifs(){
  const baseN=80000,baseL=50000;
  const base={nemausus:{caHT:baseN,months:{}},lambert:{caHT:baseL,months:{}}};
  try{
    const raw=JSON.parse(appStorage.getItem('benai_lead_obj'));
    if(!raw||typeof raw!=='object')return JSON.parse(JSON.stringify(base));
    if(raw._crmSocV3&&raw.nemausus&&raw.lambert){
      return{
        nemausus:normalizeSocObjEntry(raw.nemausus,baseN),
        lambert:normalizeSocObjEntry(raw.lambert,baseL)
      };
    }
    if(raw._crmSocV2&&raw.nemausus&&raw.lambert){
      const migrated={
        _crmSocV3:true,
        nemausus:{caHT:Math.max(0,Number(raw.nemausus.caHT)||baseN),months:{}},
        lambert:{caHT:Math.max(0,Number(raw.lambert.caHT)||baseL),months:{}}
      };
      appStorage.setItem('benai_lead_obj',JSON.stringify(migrated));
      return{
        nemausus:normalizeSocObjEntry(migrated.nemausus,baseN),
        lambert:normalizeSocObjEntry(migrated.lambert,baseL)
      };
    }
    const nH=Math.round((Number(raw.nimes?.ca)||0)+(Number(raw.avignon?.ca)||0)+(Number(raw.bagnoles?.ca)||0)+(Number(raw.zone_blanche?.ca)||0))||baseN;
    const next={_crmSocV3:true,nemausus:{caHT:nH,months:{}},lambert:{caHT:baseL,months:{}}};
    appStorage.setItem('benai_lead_obj',JSON.stringify(next));
    return{nemausus:normalizeSocObjEntry(next.nemausus,baseN),lambert:normalizeSocObjEntry(next.lambert,baseL)};
  }catch{return JSON.parse(JSON.stringify(base));}
}
function getSocieteObjectifCaHT(socId,monthKey){
  const o=getCrmSocieteObjectifs()[socId];
  if(!o)return 0;
  if(monthKey&&o.months&&Object.prototype.hasOwnProperty.call(o.months,monthKey)){
    const v=Number(o.months[monthKey]);
    return Number.isFinite(v)?Math.max(0,v):0;
  }
  return Math.max(0,Number(o.caHT)||0);
}
function saveCrmSocieteObjectifs(o){
  const norm=id=>{
    const s=o[id]||{};
    return normalizeSocObjEntry(s,id==='nemausus'?80000:50000);
  };
  appStorage.setItem('benai_lead_obj',JSON.stringify({
    _crmSocV3:true,
    nemausus:norm('nemausus'),
    lambert:norm('lambert')
  }));
}
/** Société CRM d’un lead (aligné sur la création / migration : societe_crm ou champ legacy societe). */
function getLeadCRMCompany(lead){
  if(!lead)return'nemausus';
  if(lead.societe_crm==='lambert'||lead.societe==='lambert')return'lambert';
  return'nemausus';
}
function canAccessLeadByCompany(lead){
  ...
  if(isCRMScopePilotageRole(currentUser.role)){
    ...
    const userSoc=String(currentUser.societe||'').trim().toLowerCase();
    if(userSoc==='les-deux')return true;
...
function getCrmSecteurIdsForScopeSlug(slug){
  if(slug==='nemausus')return['nimes','avignon','zone_blanche'];
  if(slug==='lambert')return['bagnoles','zone_blanche'];
...
function getLeadFormSecteurOptionIds(){
  ...
  return getCrmSecteurIdsForScopeSlug(getCrmSecteurScopeSlug());
}
...
function getBenaiPilotageSecteurShortLine(){
  const slug=getCrmSecteurScopeSlug();
  if(slug==='all')return'Les quatre zones CRM sont visibles : Nîmes, Avignon, Bagnols-sur-Cèze, zone blanche.';
  if(slug==='lambert')return'Zones visibles pour ton compte : Bagnols-sur-Cèze et zone blanche.';
  return'Zones visibles pour ton compte : Nîmes, Avignon et zone blanche.';
}
...
function renderAbsencesExportButtons(){
  const host=document.getElementById('abs-export-actions');
  if(!host)return;
  if(currentUser?.role!=='admin'){
    host.innerHTML='';
    return;
  }
  const soc=String(currentUser?.societe||'les-deux');
  const stA='background:var(--a3);border:1px solid var(--a);border-radius:8px;padding:9px 14px;font-size:12px;cursor:pointer;color:var(--a);font-family:inherit;font-weight:600';
...
/** Bandeau raccourcis sous BenAI IA pour les rôles dont l’accueil n’est pas le CRM (assistante, métreur, admin…). */
function syncBenaiQuickLinks(){
  const wrap=document.getElementById('benai-quick-links');
  if(!wrap)return;
  if(!currentUser||CRM_PAGES_ONLY.includes(currentUser.role)){
    wrap.style.display='none';
    wrap.innerHTML='';
    return;
  }
  ...
  push('annuaire','👥 Équipe');
  push('guide','📘 Guide');
  push('evolution','🚀 Évolutions');
  if(currentUser.role==='admin'){
    items.push('<button type="button" class="dash-quick-btn" onclick="showPage(\'admin\')">📊 Pilotage</button>');
    items.push('<button type="button" class="dash-quick-btn" onclick="showPage(\'bugs\')">🎫 Tickets</button>');
  }
  items.push('<button type="button" class="dash-quick-btn" onclick="openBugReportOverlay()">🛟 Signaler</button>');
  if(!items.length){
    wrap.style.display='none';
    return;
  }
...
/** Secteurs « posés » (hors zone blanche) dans le périmètre du compte — pour exports / regroupements CSV. */
function getCrmSecteurCoreIdsInScope(){
...
/** Commerciaux présents sur les leads du périmètre mais absents du filtre annuaire (évite un bloc objectifs vide). */
function enrichCommerciauxObjWithLeadAssignments(commerciauxBase,leads){
  ...
  (leads||[]).forEach(l=>{
    ...
    seen.add(idN);
    const label=String(resolveCrmCommercialLabel(raw)||raw).trim()||'Commercial';
    const iniMatch=label.match(/[A-Za-zÀ-ÿ0-9]/);
    out.push({
      id:raw,
      name:label+' (vu sur des leads)',
      role:'commercial',
      societe:'',
      vehicule:'',
      color:'linear-gradient(135deg,#64748b,#475569)',
      initial:iniMatch?iniMatch[0].toUpperCase():'?',
      _fromLeadRefs:true
    });
  });
  return out;
}
function buildCrmDashboardExportSecteurSelectHtml(){
  const slug=getCrmSecteurScopeSlug();
  const ids=getCrmSecteurIdsForScopeSlug(slug);
  const core=getCrmSecteurCoreIdsInScope();
  const parts=[];
  if(ids.length>1)parts.push(`<option value="all">${slug==='all'?'Global — les 4 secteurs':'Toutes les zones visibles pour ton compte'}</option>`);
  if(core.length>1||(slug==='all'&&core.length===3))parts.push(`<option value="core3">${slug==='all'?'Global — les 3 secteurs (hors zone blanche)':'Secteurs posés (hors zone blanche)'}</option>`);
  else if(core.length===1&&ids.includes('zone_blanche'))parts.push(`<option value="core3">${CRM_SECTOR_LABELS[core[0]]||core[0]} — hors zone blanche</option>`);
  ids.forEach(id=>{
    parts.push(`<option value="${id}">${CRM_SECTOR_LABELS[id]||id} uniquement</option>`);
  });
  return parts.join('');
}
function getDirecteurCoTutoSlides(){
  const company=getBenaiPilotageCompanyTitle();
  const zones=getBenaiPilotageSecteurShortLine();
  const slug=getCrmSecteurScopeSlug();
  const sessionHint=slug==='all'
    ?'Si un collègue te prête sa session, tu peux vérifier en haut de l’écran que le compte affiché correspond bien à ton périmètre (société / accès).'
...
function getDirecteurGeneralTutoSlides(){
  const company=getBenaiPilotageCompanyTitle();
  const zones=getBenaiPilotageSecteurShortLine();
  return[
...
// INIT PAGE LEADS — mémorise le sous-onglet CRM pour ne pas renvoyer au dashboard à chaque showPage('leads') / init.
function crmSubtabStorageKey(){
  return'benai_crm_subtab_'+normalizeId(currentUser?.id||'guest');
}
function getCrmSubtabsForRole(role){
  if(role==='admin'||isCRMScopePilotageRole(role))return['non-attribues','mes-leads','dashboard'];
  if(role==='assistante')return['mes-leads'];
  return['mes-leads','dashboard'];
}
function readStoredCrmSubtab(fallback){
  const allowed=getCrmSubtabsForRole(currentUser?.role);
  if(allowed.length===1)return allowed[0];
  try{
    const v=String(appStorage.getItem(crmSubtabStorageKey())||'').trim();
...
function initLeadsPage(){
  ...
  // Source ACTIF : commerciaux + dir. co / DG (création terrain comme un vendeur)
  const srcActif=document.getElementById('src-actif-btn');
  if(srcActif)srcActif.style.display=isCrmSalesActorRole(role)?'flex':'none';
  ...
  if(filterSecteur){
    ...
    else buildCrmPilotageSecteurFilter();
  }
  if(filterSociete){
  ...
  const sub=document.getElementById('leads-page-sub');
  if(sub)sub.textContent=getCrmWorkspaceSubtitle();
  tabs.innerHTML='';
  if(role==='admin'||isCRMScopePilotageRole(role)){
    ...
    addCRMTab('mes-leads','📋 Mes leads',crmSub==='mes-leads');
    addCRMTab('dashboard','📊 Mes ventes',crmSub==='dashboard');
    filters.style.display='flex';
    showCRMTab(crmSub);
  }
  refreshLeadsBadge();
...
function showCRMTab(id){
  try{
    if(currentUser?.id&&getCrmSubtabsForRole(currentUser.role).includes(id)){
      appStorage.setItem(crmSubtabStorageKey(),id);
    }
  }catch{}
...
function renderNonAttribues(){
  ...
  const dupBanner=isCRMScopePilotageRole(currentUser?.role)?formatDuplicatePilotageBannerHtml(getDuplicateLeadClustersForPilotage()):'';
  if(!leads.length){
    list.innerHTML=dupBanner+'<div style="color:var(--g);font-size:14px;padding:32px;text-align:center;font-weight:600">✅ Tous les leads sont attribués !</div>';
    return;
  }
  list.innerHTML=dupBanner+'<div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:var(--r);margin-bottom:10px;padding:0 2px">'+leads.length+' lead(s) en attente d\'attribution</div>'+leads.map(l=>renderDispatchCard(l)).join('');
}
...
/** Libellés sans emoji (exports, timeline). La valeur stockée reste « gris » pour compatibilité données. */
const LEAD_STATUT_PLAIN_LABELS={gris:'Non traité',rdv:'RDV pris',jaune:'Devis envoyé',vert:'Vendu',rouge:'Perdu'};
function getLeadStatutPlainLabel(statut){
  const k=String(statut==null?'':statut).trim();
  return LEAD_STATUT_PLAIN_LABELS[k]||k||'Non traité';
}
...
/** Notifications CRM leads : jamais vers admin ; pas de bruit pipeline pour DG / mètreur. */
function pushLeadCrmNotif(titre,msg,icon,uid){
  if(!uid)return;
  const target=findUserById(uid);
  if(!target)return;
  if(target.role==='admin'||target.role==='directeur_general'||target.role==='metreur')return;
  pushNotif(titre,msg,icon,uid);
}

// ATTRIBUTION RAPIDE
function dispatchLead(id){
  const sel=document.getElementById('dispatch-'+id);
  const commId=String(sel?.value||'').trim();
  if(!commId){showDriveNotif('⚠️ Choisissez un commercial ou le dir. commercial');return;}
  const leads=getLeads();
  ...
  const comm=findUserById(commId);
  if(!comm){showDriveNotif('⚠️ Commercial introuvable');return;}
  const leadSoc=getLeadCRMCompany(l);
  if(currentUser?.role!=='admin'&&!crmAssigneeCoversLeadSociete(comm,leadSoc)){
    showDriveNotif('⚠️ Ce profil n’appartient pas à la société du lead');
    return;
  }
  const dateStr=new Date().toLocaleString('fr-FR',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'});
  leads[idx].commercial=comm.id;
  addLeadTimelineEntry(leads[idx],`Attribué à ${comm?.name||commId}`,currentUser.name);
  saveLeads(leads);
  renderLeads();
  refreshLeadsBadge();
  pushLeadCrmNotif('📋 Lead assigné',`${l.nom} — ${l.type_projet}`,LEAD_SOURCE_ICONS[l.source]||'📋',comm.id);
...
function renderLeadCardAssistante(l){
  ...
  return `<div class="lead-card statut-${l.statut||'gris'}" onclick="openLead(${l.id})" style="${isLeadCrmArchivedView(l)?'opacity:.6;':''}">
...
/** Pastille « secteur terrain » (code postal) — distinct du dossier CRM ; texte + infobulle explicites. */
function formatLeadSecteurGeographiqueChipHtml(l){
  if(!l)return'';
  const lab=getLeadSecteurLabel(l.secteur);
  const baseTip=`Zone géographique déduite du code postal : « ${lab} ». Indique le secteur terrain, pas l’entreprise qui porte le dossier dans le CRM.`;
  let tip=baseTip;
  if(isLeadCrossEntitySecteurAnomaly(l)){
...
function getArchivedLeadsStats(leads){
  const arch=(leads||[]).filter(l=>isLeadCrmArchivedView(l));
...
function buildCrmCommercialEvolutionCards(leadsActifs,commerciaux){
  ...
  return commerciaux.map(c=>{
    const rows=months.map(m=>{
      ...
      const ca=sales.reduce((s,l)=>s+Number(l.prix_vendu||0),0);
      return`<div class="crm-evo-row"><span style="color:var(--t3);min-width:54px">${formatMois(m)}</span><span style="font-weight:600;color:var(--g);text-align:right;flex:1">${ca.toLocaleString('fr-FR')} €</span><span style="color:var(--t2);font-size:10px">${sales.length} v.</span></div>`;
    }).join('');
    return`<div class="crm-evo-card"><div class="crm-evo-head"><span style="width:26px;height:26px;border-radius:7px;background:${c.color};display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;color:#fff">${c.initial}</span>${esc(c.name)}</div>${rows}</div>`;
  }).join('');
}

// DASHBOARD — mémorise les <details> ouverts pour ne pas les réouvrir au prochain rendu (sync / polling).
function captureCrmDashboardDetailsOpenState(){
  const list=document.getElementById('leads-list');
  if(!list)return undefined;
  const panels=[...list.querySelectorAll('details.crm-dash-grp[id]')];
  if(!panels.length)return undefined;
  const open=new Set();
  panels.forEach(d=>{if(d.open)open.add(d.id);});
  return open;
}
function applyCrmDashboardDetailsOpenState(openIds){
  const list=document.getElementById('leads-list');
  if(!list)return;
  const panels=[...list.querySelectorAll('details.crm-dash-grp[id]')];
  if(!panels.length)return;
  const role=currentUser?.role||'';
  const pilot=isCRMScopePilotageRole(role);
  if(openIds===undefined||openIds===null){
    panels.forEach(d=>{
      if(pilot&&d.id==='crm-dash-objectifs')d.open=true;
      else if(!pilot&&d.id==='crm-dash-secteurs')d.open=true;
      else d.open=false;
    });
    return;
  }
  panels.forEach(d=>{d.open=openIds.has(d.id);});
}

/** Après sync cloud : met à jour l’onglet CRM actif sans tout reconstruire (évite fermeture des accordéons). */
function refreshLeadsPageIfVisible(){
  if(document.getElementById('page-leads')?.style.display!=='flex')return;
  const tabs=document.getElementById('crm-tabs');
  if(!tabs||!tabs.querySelector('.crm-tab'))return;
  const active=document.querySelector('#crm-tabs .crm-tab.active');
  const tid=String(active?.id||'').replace(/^crm-tab-/,'');
  if(tid==='dashboard')renderLeadsDashboard();
  else if(tid==='non-attribues')renderNonAttribues();
  else renderLeads();
}
...
function renderLeadsDashboard(){
  ...
  const socMonthKeys=getObjCommMonthOptions();
  let selSocMois=appStorage.getItem('benai_lead_obj_soc_mois')||getDefaultObjCommMonthKey();
  if(!socMonthKeys.includes(selSocMois))selSocMois=getDefaultObjCommMonthKey();
  appStorage.setItem('benai_lead_obj_soc_mois',selSocMois);
  const socParts=selSocMois.split('-').map(Number);
  const socY=socParts[0],socM=socParts[1];
  const leadsMonthSoc=leads.filter(l=>{const d=new Date(l.date_creation||0);return!isNaN(d.getTime())&&d.getMonth()===socM-1&&d.getFullYear()===socY;});
  const now=new Date();
  ...
  if(!dashCommercial){
    const activeIds=new Set(commerciaux.map(c=>normalizeId(c.id)));
    const objKeys=new Set(Object.keys(getObjectifsCommerciaux()).map(normalizeId));
    getCommercialArchives().forEach(a=>{
      ...
      commerciauxObj.push({
        ...
        societe:a.societe||'',
        vehicule:a.vehicule||'',
        color:a.color||'linear-gradient(135deg,#888,#555)',
        initial:a.initial||'?',
        _archivedCommercial:true
      });
    });
  }
  const secteurScopeSlug=getCrmSecteurScopeSlug();
  const secteurScopeSet=new Set(getCrmSecteurIdsForScopeSlug(secteurScopeSlug));
  const secteursAll=[
  ...
  if(crmDashPilotDirCo){
    html+=formatDuplicatePilotageBannerHtml(getDuplicateLeadClustersForPilotage(leads));
  }
  html+=`<div class="crm-dash-kicker">${dashCommercial?'Mes ventes':'Tableau de bord CRM'}</div>`;
  html+=`<div class="crm-dash-grid-kpis" style="margin-bottom:12px${dashCommercial?';grid-template-columns:repeat(2,minmax(0,1fr))':''}">
    <div class="stat-box" title="Tous les leads actifs (hors archives) dans votre périmètre"><div class="stat-box-val">${totalLeads}</div><div class="stat-box-lbl">Leads actifs</div></div>
    ${dashCommercial?'':`<div class="stat-box" title="Leads non encore attribués à un commercial"><div class="stat-box-val" style="color:var(--y)">${nonAttrib}</div><div class="stat-box-lbl">À attribuer</div></div>`}
    <div class="stat-box" title="Leads nécessitant une action prioritaire"><div class="stat-box-val" style="color:var(--r)">${alertes}</div><div class="stat-box-lbl">Alertes</div></div>
  </div>`;
  if(dashPilotage){
    const scopeHint=secteurScopeSlug==='all'?'':`<div style="font-size:12px;color:var(--t2);margin:0 0 8px;line-height:1.55"><strong>Périmètre compte :</strong> ${esc(getBenaiPilotageCompanyTitle())}. ${esc(getBenaiPilotageSecteurShortLine())}</div>`;
    html+=`<div class="crm-dash-intro">${scopeHint}<strong>Mode d’emploi</strong> — les trois cases ci-dessus résument la situation. Tu peux t’aider de la barre <strong>Aller à</strong> pour aller à une section ; chaque bloc se replie ou se déplie via sa ligne de titre.${crmDashPilotDirCo?' Au chargement, <strong>Objectifs commerciaux</strong> est souvent ouvert et <strong>Secteurs & mois</strong> replié pour mettre en avant le suivi CA vendeur — tu peux tout rouvrir ou refermer à ta convenance.':' Au chargement, seul <strong>Secteurs & mois</strong> est souvent ouvert pour une première vue lisible — tu peux ajuster les replis comme tu préfères.'}</div>`;
  } else if(dashCommercial){
    html+=`<div class="crm-dash-intro">Ici : ton <strong>tableau de ventes</strong> (détail par mois, comparaison N-1) et <strong>tes objectifs</strong>. Les vues secteurs, chiffres par secteur, RDV agrégés et analyses de pertes sont surtout proposées à la direction commerciale, au DG et à l’administration.</div>`;
  }
  const rapportHead=genererRapportAlertes();
  if(rapportHead.total>0){
  ...
  if(dashPilotage){
    html+=`<nav class="crm-dash-navstrip" aria-label="Sections du tableau de bord CRM">
      <div class="crm-dash-navstrip-inner">
        <span class="crm-dash-navstrip-label">Aller à</span>
        <a class="crm-dash-navlink" href="#crm-dash-secteurs">Secteurs & mois</a>
        <a class="crm-dash-navlink" href="#crm-dash-tab-mois">Tableau du mois</a>
        <a class="crm-dash-navlink" href="#crm-dash-ventes-rdv">Ventes &amp; RDV</a>
        <a class="crm-dash-navlink" href="#crm-dash-ca-soc">${entPilotageList.length>1?'CA par entité':'CA'}</a>
        <a class="crm-dash-navlink" href="#crm-dash-detail-ventes">Détail ventes</a>
        <a class="crm-dash-navlink" href="#crm-dash-equipe">Équipe</a>
        <a class="crm-dash-navlink" href="#crm-dash-objectifs">Objectifs</a>
        <a class="crm-dash-navlink" href="#crm-dash-analyse">Pertes &amp; archives</a>
        <a class="crm-dash-navlink" href="#crm-dash-donnees">Exports</a>
      </div>
    </nav>`;
  } else if(dashCommercial){
    html+=`<nav class="crm-dash-navstrip" aria-label="Sections Mes ventes">
      <div class="crm-dash-navstrip-inner">
        <span class="crm-dash-navstrip-label">Aller à</span>
        <a class="crm-dash-navlink" href="#crm-dash-detail-ventes">Tableau de ventes</a>
        <a class="crm-dash-navlink" href="#crm-dash-mes-obj">Mes objectifs</a>
      </div>
    </nav>`;
  }
  if(dashCommercial){
    html+=`<div id="crm-dash-detail-ventes" class="crm-dash-anchor" style="margin-top:12px"></div>`;
  }
  const secteursSynthese=!dashCommercial?secteurs.map(s=>{
  ...
  const moisSecteurLabel=now.toLocaleDateString('fr-FR',{month:'long',year:'numeric'});
  const secteurMensuelRows=!dashCommercial?secteurs.map(s=>{
    const lm=leadsMonth.filter(l=>l.secteur===s.id);
    const gris=lm.filter(l=>l.statut==='gris').length;
    const rdv=lm.filter(l=>l.statut==='rdv').length;
    const jaune=lm.filter(l=>l.statut==='jaune').length;
    const vert=lm.filter(l=>l.statut==='vert').length;
    const rouge=lm.filter(l=>l.statut==='rouge').length;
    const ventesLm=lm.filter(l=>l.statut==='vert');
    const caHt=ventesLm.reduce((sum,l)=>sum+Number(l.prix_vendu||0),0);
    const devisLm=lm.filter(l=>l.statut==='jaune');
    const pipeHt=devisLm.reduce((sum,l)=>sum+Number(l.montant_devis||0),0);
    return{s,lm,gris,rdv,jaune,vert,rouge,caHt,pipeHt,ndevis:devisLm.length};
  }):[];
  const totLeadsM=secteurMensuelRows.reduce((a,r)=>a+r.lm.length,0);
  const totVertM=secteurMensuelRows.reduce((a,r)=>a+r.vert,0);
  const totCaM=secteurMensuelRows.reduce((a,r)=>a+r.caHt,0);
  const totPipeM=secteurMensuelRows.reduce((a,r)=>a+r.pipeHt,0);
  const totDevisM=secteurMensuelRows.reduce((a,r)=>a+r.ndevis,0);
  const htmlSecteurTable=!dashCommercial?`<div id="crm-dash-tab-mois" class="secteur-card crm-dash-anchor">
    <div class="secteur-title" style="font-size:14px">📊 ${moisSecteurLabel.charAt(0).toUpperCase()+moisSecteurLabel.slice(1)} — compteur leads & chiffres par secteur</div>
    <div style="font-size:11px;color:var(--t3);margin:-6px 0 10px;line-height:1.45">Leads <strong>créés ce mois</strong> (date de création) dans votre périmètre, ventes & CA HT du même lot, pipeline des devis « jaune ».</div>
    <div class="crm-sect-m-wrap">
      <table class="crm-sect-m-table">
        <thead><tr>
          <th>Secteur</th>
          <th class="num">Leads<br><span style="font-weight:400;opacity:.85">(mois)</span></th>
          <th class="num">Non traité</th>
          <th class="num">RDV</th>
          <th class="num">Devis</th>
          <th class="num" title="Ventes signées (statut vert)">Vendus</th>
          <th class="num">Perdus</th>
          <th class="num">CA HT (€)</th>
          <th class="num">Fiches devis</th>
          <th class="num">Pipeline HT (€)</th>
        </tr></thead>
        <tbody>
          ${secteurMensuelRows.map(r=>`<tr>
            <td class="sect" style="color:${r.s.color}">${r.s.label}</td>
            <td class="num">${r.lm.length}</td>
            <td class="num">${r.gris}</td>
            <td class="num">${r.rdv}</td>
            <td class="num">${r.jaune}</td>
            <td class="num" style="color:var(--g)">${r.vert}</td>
            <td class="num" style="color:var(--r)">${r.rouge}</td>
            <td class="num" style="color:var(--g);font-weight:600">${r.caHt.toLocaleString('fr-FR')}</td>
            <td class="num">${r.ndevis}</td>
            <td class="num" style="color:var(--y);font-weight:600">${r.pipeHt.toLocaleString('fr-FR')}</td>
          </tr>`).join('')}
          <tr class="crm-sect-m-tot">
            <td class="sect">Total</td>
            <td class="num">${totLeadsM}</td>
            <td class="num">${secteurMensuelRows.reduce((a,r)=>a+r.gris,0)}</td>
            <td class="num">${secteurMensuelRows.reduce((a,r)=>a+r.rdv,0)}</td>
            <td class="num">${secteurMensuelRows.reduce((a,r)=>a+r.jaune,0)}</td>
            <td class="num" style="color:var(--g)">${totVertM}</td>
            <td class="num" style="color:var(--r)">${secteurMensuelRows.reduce((a,r)=>a+r.rouge,0)}</td>
            <td class="num" style="color:var(--g)">${totCaM.toLocaleString('fr-FR')}</td>
            <td class="num">${totDevisM}</td>
            <td class="num" style="color:var(--y)">${totPipeM.toLocaleString('fr-FR')}</td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  if(!dashCommercial){
    html+=`<details class="crm-dash-grp crm-dash-anchor" id="crm-dash-secteurs">${crmDashSummary('📍 Secteurs & mois en cours','Carte par zone, export CSV, puis tableau détaillé du mois civil.')}<div class="crm-dash-grp-body">
    <div class="crm-dash-subtle">D’abord la vue « toutes périodes » et l’export ; puis le tableau ne compte que les <strong>leads créés ce mois</strong> (date de création).</div>
    <div class="secteur-card"><div class="secteur-title" style="font-size:13px">Export & résumé par zone</div>
    ...
    <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin:0 0 10px">
      <label style="font-size:11px;color:var(--t3);display:flex;align-items:center;gap:6px;flex:1;min-width:200px">
        ...
        <select id="crm-dash-export-secteur" class="form-input" style="flex:1;min-width:160px;padding:6px 10px;font-size:12px;border-radius:8px">${buildCrmDashboardExportSecteurSelectHtml()}</select>
      </label>
    ...
    <div style="font-size:11px;color:var(--t3);margin:-4px 0 10px;line-height:1.45">Leads <strong>actifs non archivés</strong> du périmètre visible. Colonnes CSV : secteur, leads, ventes, CA signé, devis en cours, pipeline devis.</div>
  ...
  if(dashPilotage){
    html+=`<details class="crm-dash-grp crm-dash-anchor" id="crm-dash-ventes-rdv">${crmDashSummary('💼 Ventes, RDV & canaux','Totaux globaux, RDV du mois, origine des leads, puis liste détaillée des ventes.')}<div class="crm-dash-grp-body"><div class="crm-dash-subtle">Les totaux « ventes » portent sur toutes les ventes signées ; les encarts « ce mois » utilisent le <strong>mois civil en cours</strong>.</div>`;
  }
  html+=`<div class="secteur-card">
    <div class="secteur-title">💼 Chiffres clés ventes</div>
  ...
  html+=`<div class="secteur-card">
    <div class="secteur-title">📅 RDV réalisés ce mois</div>
  ...
  html+=`<div class="crm-dash-kicker k-pr" style="display:flex;flex-wrap:wrap;align-items:center;gap:10px;justify-content:space-between;row-gap:8px;margin-top:0">
    ...
    <label style="display:flex;align-items:center;gap:6px;font-size:11px;color:var(--t2);font-weight:600;font-family:inherit">Mois affiché
      <select id="crm-obj-soc-mois" class="form-input" onchange="window.appStorage.setItem('benai_lead_obj_soc_mois',this.value);renderLeadsDashboard()" style="padding:5px 10px;font-size:11px;border-radius:8px;min-width:140px">
        ${socMonthKeys.map(m=>`<option value="${m}"${m===selSocMois?' selected':''}>${formatMois(m)}</option>`).join('')}
      </select>
    </label>
  </div>
  <div style="font-size:11px;color:var(--t3);margin:-4px 0 10px;line-height:1.45">Chiffres du <strong>${formatMois(selSocMois)}</strong> (création des leads). Passez au même mois en <strong>${year-1}</strong> pour le comparer à l’instant. Objectif : montant saisi pour ce mois, sinon <strong>objectif de référence</strong> (valeur par défaut entreprise).</div>`;
  const socCanEdit=currentUser.role==='admin'||isCRMScopePilotageRole(currentUser.role);
  html+=`<div class="crm-ent-grid">`;
  entPilotageList.forEach(ent=>{
    const sl=leadsMonthSoc.filter(l=>getLeadCRMCompany(l)===ent.id);
    ...
    const oCA=getSocieteObjectifCaHT(ent.id,selSocMois);
    const pCA=oCA>0?Math.min(ca/oCA*100,110):0;
    html+=`<div class="crm-ent-card">
      <div class="secteur-title" style="color:${ent.color};margin-bottom:10px;font-size:14px">${ent.label}
        ${socCanEdit?`<button type="button" onclick="editObjectifSociete('${ent.id}')" title="Définir ou modifier l'objectif CA HT pour le mois sélectionné (${formatMois(selSocMois)})" style="margin-left:auto;background:var(--s2);border:1px solid var(--b1);border-radius:6px;padding:4px 10px;font-size:11px;cursor:pointer;font-family:inherit;color:var(--t2);font-weight:600">⚙️ Objectif CA HT</button>`:''}
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
        <div class="stat-box"><div class="stat-box-val" style="font-size:17px">${total}</div><div class="stat-box-lbl">Leads (mois)</div></div>
        <div class="stat-box"><div class="stat-box-val" style="font-size:17px;color:var(--g)">${ventes.length}</div><div class="stat-box-lbl">Ventes signées</div></div>
      </div>
  ...
  if(!dashCommercial){
  if(dashPilotage){
    html+=`<details class="crm-dash-grp crm-dash-anchor" id="crm-dash-equipe">${crmDashSummary('👥 Équipe commerciale','Classement du mois et courbe de CA sur six mois par vendeur.')}<div class="crm-dash-grp-body">`;
  }
  ...
  if(!ranking.length){
    ...
    html+=ranking.map((r,i)=>{
      return `<div class="perf-row">
        ...
        <div class="perf-name">${esc(resolveCrmCommercialLabel(r.uid))}</div>
  ...
  html+='</div>';
  const commerciauxEvoCharts=[...commerciaux,...commerciauxObj.filter(c=>c._archivedCommercial||c._fromLeadRefs)];
  if(commerciauxEvoCharts.length){
    html+=`<div class="secteur-card"><div class="secteur-title">📈 Évolution CA par commercial (6 mois)</div>
    <div style="font-size:11px;color:var(--t3);margin:-4px 0 4px">Montants HT — ventes signées (date de création du lead).</div>
    <div class="crm-evo-grid">${buildCrmCommercialEvolutionCards(leads,commerciauxEvoCharts)}</div></div>`;
  }
  if(dashPilotage){
    html+=`</div></details>`;
  }
  if(dashPilotage){
    const objCommMonthKeysP=getObjCommMonthOptions();
    let selObjCommP=appStorage.getItem('benai_obj_comm_mois')||getDefaultObjCommMonthKey();
    if(!objCommMonthKeysP.includes(selObjCommP))selObjCommP=getDefaultObjCommMonthKey();
    appStorage.setItem('benai_obj_comm_mois',selObjCommP);
    html+=`<details class="crm-dash-grp crm-dash-anchor" id="crm-dash-objectifs">${crmDashSummary('🎯 Objectifs commerciaux','CA réalisé vs cible : mois N et N-1 pour comparer à l’année précédente.')}<div class="crm-dash-grp-body"><div class="crm-dash-subtle">L’objectif <strong>mensuel</strong> est celui du mois affiché. Vous pouvez <strong>répartir le mois en objectifs par semaine ISO</strong> (modifiables) ; sinon l’<strong>objectif hebdo global</strong> sert de cible pour la semaine en cours. Mois disponibles : <strong>${year-1}</strong> et <strong>${year}</strong>. Les vendeurs listés incluent l’annuaire <em>et</em> les attributions visibles sur les leads du périmètre.</div>`;
    html+=`<div class="secteur-card"><div class="secteur-title" style="display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:10px">
      <span>🎯 Suivi & objectifs commerciaux</span>
      <label style="display:flex;align-items:center;gap:6px;font-size:11px;color:var(--t3);font-weight:600">Mois affiché
        <select id="crm-obj-comm-mois" class="form-input" onchange="window.appStorage.setItem('benai_obj_comm_mois',this.value);renderLeadsDashboard()" style="padding:5px 10px;font-size:11px;border-radius:8px;min-width:128px">
          ${objCommMonthKeysP.map(m=>`<option value="${m}"${m===selObjCommP?' selected':''}>${formatMois(m)}</option>`).join('')}
        </select>
      </label>
    </div>`;
    if(!commerciauxObj.length){
      html+=`<div style="background:var(--s3);border-radius:10px;padding:14px;margin-bottom:8px;font-size:12px;color:var(--t2);line-height:1.55;border:1px dashed var(--b1)">Aucun commercial dans la liste : soit aucun compte vendeur ne correspond à ton périmètre, soit aucun lead attribué n’est visible. Vérifie les comptes dans l’administration ou ouvre <strong>Tous les leads</strong> pour contrôler les attributions. Tu peux quand même saisir des objectifs dès qu’un commercial apparaît (bouton ⚙️ sur chaque ligne).</div>`;
    }else{
      commerciauxObj.forEach(c=>{
        const prog=getProgressionCommercial(c.id,selObjCommP);
        const moisHuman=formatMois(selObjCommP);
        ...
        html+=`<div style="background:var(--s3);border-radius:10px;padding:12px;margin-bottom:8px">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
            ...
            ${!c._archivedCommercial&&(currentUser.role==='admin'||isCRMScopePilotageRole(currentUser.role))?`<button type="button" class="crm-obj-comm-edit" data-uid="${escAttr(String(c.id??''))}" title="Modifier les objectifs CA" style="background:var(--s2);border:1px solid var(--b1);border-radius:6px;padding:3px 8px;font-size:10px;cursor:pointer;font-family:inherit;color:var(--t2)">⚙️</button>`:''}
          </div>
          <div style="font-size:10px;color:var(--t3);margin-bottom:3px">Mensuel (${moisHuman}) — ${prog.ca_mois.toLocaleString('fr-FR')}€ / ${prog.obj.mensuel.toLocaleString('fr-FR')}€ · ${prog.ventes_mois} ventes</div>
          ...
          <div style="font-size:10px;color:var(--t3);margin-bottom:3px">Hebdo${prog._objWeekKey?' ('+esc(String(prog._objWeekKey).replace('-',' '))+')':''} — ${prog.ca_semaine.toLocaleString('fr-FR')}€ / ${prog.obj.hebdo.toLocaleString('fr-FR')}€ · ${prog.ventes_semaine} ventes</div>
          ...
          ${pctMois>=100?`<div style="margin-top:6px;font-size:11px;color:var(--g);font-weight:600">🏆 Objectif mensuel dépassé (${moisHuman}) !</div>`:''}
        </div>`;
      });
    }
  ...
  // Analyse leads perdus & archives (pilotage uniquement)
  if(dashPilotage){
    html+=`<details class="crm-dash-grp crm-dash-anchor" id="crm-dash-analyse">${crmDashSummary('📋 Pertes & archives','Motifs des leads perdus ce mois, puis volumes (archives manuelles + leads d’années passées).')}<div class="crm-dash-grp-body">`;
  ...
  html+=`<div class="secteur-card"><div class="secteur-title">🗃️ Historique archivé</div>
    ...
    <div style="margin-top:8px;font-size:11px;color:var(--t3)">Filtre <strong>📁 Archives</strong> dans Leads : archives manuelles <em>et</em> leads créés avant <strong>${new Date().getFullYear()}</strong>.</div>
  </div>`;
    html+=`</div></details>`;
  }
  if(role==='admin'||isCRMScopePilotageRole(role)){
    if(dashPilotage){
      html+=`<details class="crm-dash-grp crm-dash-anchor" id="crm-dash-donnees">${crmDashSummary('📤 Fichiers & outils','Téléchargements CSV / Excel et fusion des doublons.')}<div class="crm-dash-grp-body">`;
    } else {
      html+=`<div class="crm-dash-kicker k-ex">Exports & actions</div>`;
    }
    ...
    <div style="font-size:10px;color:var(--t3);margin:-4px 0 12px;line-height:1.45">Les exports CSV sont en <strong>UTF-8</strong> avec séparateur <strong>;</strong> (réglage Excel France). Double-cliquez le fichier : les colonnes et les caractères accentués doivent s’afficher correctement.</div>`;
    html+=`<div class="secteur-card">
    ...
    if(role==='admin'&&dashPilotage){
      ...
      <div class="secteur-title" style="color:var(--r)">🧨 Remise à zéro (production / fin de tests)</div>
      <div style="font-size:11px;color:var(--t2);line-height:1.55;margin-bottom:10px">Efface <strong>toutes</strong> les données CRM cloud listées ci-dessus, le fil de messages internes synchronisé, puis <strong>supprime tous les comptes</strong> sauf le vôtre. Vous devez saisir votre <strong>mot de passe cloud</strong> (connexion admin). Déployez la fonction <code style="font-size:10px">admin-reset-demo-data</code> et exécutez <code style="font-size:10px">patch_admin_reset_demo_data_rpc.sql</code> une fois sur Supabase.</div>
      <button type="button" onclick="pilotageAdminResetDemoData()" style="padding:10px 14px;background:var(--r);color:#fff;border:none;border-radius:8px;font-family:inherit;font-size:12px;font-weight:700;cursor:pointer">Supprimer données + autres comptes</button>
    </div>`;
    }
    if(dashPilotage){
      html+=`</div></details>`;
  ...
  // OBJECTIFS — vue commercial uniquement (dir. co / DG / admin : bloc plus haut après Équipe)
  if(dashCommercial&&commerciauxObj.length>0){
    const objCommMonthKeys=getObjCommMonthOptions();
    let selObjComm=appStorage.getItem('benai_obj_comm_mois')||getDefaultObjCommMonthKey();
    if(!objCommMonthKeys.includes(selObjComm))selObjComm=getDefaultObjCommMonthKey();
    appStorage.setItem('benai_obj_comm_mois',selObjComm);
    html+=`<div class="crm-dash-kicker k-pr" style="margin-top:8px">Mes objectifs & suivi CA</div>`;
    html+=`<div class="secteur-card crm-dash-anchor" id="crm-dash-mes-obj"><div class="secteur-title" style="display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:10px">
      <span>🎯 Mes objectifs (mois N et N-1 pour comparer)</span>
      <label style="display:flex;align-items:center;gap:6px;font-size:11px;color:var(--t3);font-weight:600">Mois affiché
        <select id="crm-obj-comm-mois" class="form-input" onchange="window.appStorage.setItem('benai_obj_comm_mois',this.value);renderLeadsDashboard()" style="padding:5px 10px;font-size:11px;border-radius:8px;min-width:128px">
          ${objCommMonthKeys.map(m=>`<option value="${m}"${m===selObjComm?' selected':''}>${formatMois(m)}</option>`).join('')}
        </select>
      </label>
    </div>`;
    commerciauxObj.forEach(c=>{
      const prog=getProgressionCommercial(c.id,selObjComm);
      const moisHuman=formatMois(selObjComm);
      const pctMois=prog.obj.mensuel>0?Math.min(prog.ca_mois/prog.obj.mensuel*100,110):0;
      const pctHebdo=prog.obj.hebdo>0?Math.min(prog.ca_semaine/prog.obj.hebdo*100,110):0;
      const contacts=getContactsParCommercial()[c.id]||{ce_mois:0,cette_semaine:0};
      html+=`<div style="background:var(--s3);border-radius:10px;padding:12px;margin-bottom:8px">
  ...
  // TABLEAU DE VENTES
  if(list){
    list.style.maxWidth='920px';
    list.style.margin='0 auto';
    list.style.width='100%';
    list.innerHTML=html;
    list.querySelectorAll('.crm-obj-comm-edit').forEach(btn=>{
      btn.addEventListener('click',ev=>{
        ev.preventDefault();
        const uid=btn.getAttribute('data-uid');
        if(uid)editObjectifCommercial(uid);
      });
    });
    applyCrmDashboardDetailsOpenState(dashOpenIds);
    const slot=document.getElementById('crm-dash-slot-ventes');
    const detailVentes=document.getElementById('crm-dash-detail-ventes');
    if(slot){
      renderTableauVentes(slot);
    } else if(detailVentes){
      renderTableauVentes(detailVentes);
    } else {
  const tvDiv=document.createElement('div');tvDiv.className='secteur-card';
  renderTableauVentes(tvDiv);
      list.appendChild(tvDiv);
    }
  }
}

function editObjectifCommercial(uid,name){
  document.getElementById('obj-comm-edit-overlay')?.remove();
  const nameLabel=(name!=null&&String(name).trim()!=='')?String(name):resolveCrmCommercialLabel(uid);
  const monthKey=(document.getElementById('crm-obj-comm-mois')?.value)||getDefaultObjCommMonthKey();
  const all=getObjectifsCommerciaux();
  const curEnt=normalizeCommercialObjEntry(all[uid]||{});
  const curM=curEnt.months[monthKey]?.mensuel??0;
  const curH=Number(curEnt.hebdo)||0;
  const labelMois=formatMois(monthKey);
  const parts=monthKey.split('-').map(Number);
  const yM=parts[0],moM=parts[1];
  const weekKeys=Number.isFinite(yM)&&Number.isFinite(moM)?collectIsoWeekKeysInMonth(yM,moM):[];
  const weeksSaved=curEnt.months[monthKey]?.weeks||{};
  const mensuelNum=Number(curM)||0;
  const baseEach=weekKeys.length&&mensuelNum>0?Math.floor(mensuelNum/weekKeys.length):0;
  const weekRows=weekKeys.map(wk=>{
    const raw=weeksSaved[wk];
    const val=Number(raw)>0?Number(raw):baseEach;
    return`<div class="form-group" style="margin-bottom:8px">
      <div class="form-label" style="font-size:11px">${esc(wk)}</div>
      <input type="number" class="form-input obj-comm-wk" data-wk="${escAttr(wk)}" min="0" step="100" value="${val||0}" inputmode="decimal" style="font-family:inherit">
    </div>`;
  }).join('');
  const wrap=document.createElement('div');
  wrap.id='obj-comm-edit-overlay';
  wrap.className='lead-modal open';
  wrap.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:4500;display:flex;align-items:center;justify-content:center;padding:16px';
  wrap.innerHTML=`
    <div class="lead-modal-box" style="max-width:520px;max-height:92vh;overflow-y:auto" role="dialog" aria-modal="true" aria-labelledby="obj-comm-edit-title" onclick="event.stopPropagation()">
      <div class="lead-modal-header">
        <div>
          <div id="obj-comm-edit-title" style="font-size:16px;font-weight:700">Objectifs CA — ${esc(nameLabel)}</div>
          <div style="font-size:11px;color:var(--t3);margin-top:4px;line-height:1.45">Mois : <strong>${esc(labelMois)}</strong> · pour un autre mois, change d’abord « Mois affiché » sur le tableau de bord CRM.</div>
        </div>
        <button type="button" id="obj-comm-close-x" style="background:transparent;border:none;color:var(--t2);font-size:20px;line-height:1;cursor:pointer;padding:4px 8px" aria-label="Fermer">×</button>
      </div>
      <div class="lead-modal-body">
        <div class="lead-modal-section">
          <div class="lead-modal-section-title">Objectif mensuel (€ HT)</div>
          <div class="form-group">
            <div class="form-label">Cible pour ${esc(labelMois)}</div>
            <input type="number" class="form-input" id="obj-comm-mensuel" min="0" step="100" value="${Number(curM)||0}" inputmode="decimal" style="font-family:inherit">
          </div>
        </div>
        <div class="lead-modal-section">
          <div class="lead-modal-section-title">Objectif hebdomadaire global (€ HT)</div>
          <div class="form-group">
            <div class="form-label">Cible par défaut quand aucune semaine du mois n’a de montant saisi ci-dessous</div>
            <input type="number" class="form-input" id="obj-comm-hebdo" min="0" step="100" value="${Number(curH)||0}" inputmode="decimal" style="font-family:inherit">
          </div>
        </div>
        <div class="lead-modal-section">
          <div class="lead-modal-section-title">Objectifs par semaine (ISO, ${esc(labelMois)})</div>
          <div class="form-group">
            <div class="form-label" style="font-size:11px;color:var(--t3)">Montants € HT par semaine calendaire intersectant ce mois. Priorité sur le global pour le suivi « semaine en cours ».</div>
            <button type="button" id="obj-comm-split-weeks" style="margin-top:8px;margin-bottom:4px;padding:8px 12px;background:var(--s3);border:1px solid var(--b1);border-radius:8px;font-size:12px;cursor:pointer;font-family:inherit;color:var(--t1);font-weight:600">⚖️ Répartir le mensuel à parts égales</button>
          </div>
          ${weekRows||'<div style="font-size:12px;color:var(--t3)">Impossible de lister les semaines pour ce mois.</div>'}
        </div>
        <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:8px;flex-wrap:wrap">
          <button type="button" id="obj-comm-cancel" style="padding:10px 16px;background:var(--s3);color:var(--t1);border:1px solid var(--b1);border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit">Annuler</button>
          <button type="button" id="obj-comm-save" style="padding:10px 16px;background:linear-gradient(135deg,var(--a),var(--a2));color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">Enregistrer</button>
        </div>
      </div>
    </div>`;
  const close=()=>{
  ...
  document.addEventListener('keydown',onKey);
  wrap.addEventListener('click',e=>{if(e.target===wrap)close();});
  wrap.querySelector('#obj-comm-close-x')?.addEventListener('click',close);
  wrap.querySelector('#obj-comm-cancel')?.addEventListener('click',close);
  wrap.querySelector('#obj-comm-split-weeks')?.addEventListener('click',()=>{
    const mEl=document.getElementById('obj-comm-mensuel');
    const m=Math.max(0,Number(String(mEl?.value||'0').replace(',','.'))||0);
    const inputs=[...wrap.querySelectorAll('.obj-comm-wk')];
    if(!inputs.length||!m)return;
    const each=Math.floor(m/inputs.length);
    let rest=m-each*inputs.length;
    inputs.forEach((el,i)=>{
      let v=each+(i===0?rest:0);
      el.value=String(v);
    });
  });
  wrap.querySelector('#obj-comm-save')?.addEventListener('click',()=>{
    const mensuel=document.getElementById('obj-comm-mensuel')?.value;
    const hebdo=document.getElementById('obj-comm-hebdo')?.value;
    const weeks={};
    wrap.querySelectorAll('.obj-comm-wk').forEach(el=>{
      const wk=el.getAttribute('data-wk');
      if(!wk)return;
      const n=Number(String(el.value||'').replace(/\s/g,'').replace(',','.'));
      if(Number.isFinite(n)&&n>0)weeks[wk]=n;
    });
    setObjectifCommercial(uid,monthKey,mensuel,hebdo,weeks);
  renderLeadsDashboard();
    logActivity(`${currentUser.name} a mis à jour les objectifs de ${nameLabel} (${labelMois})`);
    close();
  });
  document.body.appendChild(wrap);
  const inp=document.getElementById('obj-comm-mensuel');
  if(inp){inp.focus();try{inp.select();}catch(e){}}
}
...
function editObjectifSociete(socId){
  ...
  const ent=next[socId]||{caHT:socId==='lambert'?50000:80000,months:{}};
  if(!ent.months||typeof ent.months!=='object')ent.months={};
  const hasM=Object.prototype.hasOwnProperty.call(ent.months,monthKey);
  const cur=hasM?Number(ent.months[monthKey]):Number(ent.caHT)||0;
  document.getElementById('obj-soc-edit-overlay')?.remove();
  const wrap=document.createElement('div');
  wrap.id='obj-soc-edit-overlay';
  wrap.className='lead-modal open';
  wrap.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:4500;display:flex;align-items:center;justify-content:center;padding:16px';
  wrap.innerHTML=`
    <div class="lead-modal-box" style="max-width:440px" role="dialog" aria-modal="true" aria-labelledby="obj-soc-edit-title" onclick="event.stopPropagation()">
      <div class="lead-modal-header">
        <div>
          <div id="obj-soc-edit-title" style="font-size:16px;font-weight:700">Objectif CA HT — ${esc(label)}</div>
          <div style="font-size:11px;color:var(--t3);margin-top:4px;line-height:1.45">Mois : <strong>${esc(labelMois)}</strong> · pour un autre mois, change d’abord « Mois affiché » sur le tableau de bord CRM.</div>
        </div>
        <button type="button" id="obj-soc-close-x" style="background:transparent;border:none;color:var(--t2);font-size:20px;line-height:1;cursor:pointer;padding:4px 8px" aria-label="Fermer">×</button>
      </div>
      <div class="lead-modal-body">
        <div class="lead-modal-section">
          <div class="form-group">
            <div class="form-label">Montant cible (€ HT)</div>
            <input type="number" class="form-input" id="obj-soc-ca" min="0" step="100" value="${Number(cur)||0}" inputmode="decimal" style="font-family:inherit">
          </div>
        </div>
        <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:8px;flex-wrap:wrap">
          <button type="button" id="obj-soc-cancel" style="padding:10px 16px;background:var(--s3);color:var(--t1);border:1px solid var(--b1);border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit">Annuler</button>
          <button type="button" id="obj-soc-save" style="padding:10px 16px;background:linear-gradient(135deg,var(--a),var(--a2));color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit">Enregistrer</button>
        </div>
      </div>
    </div>`;
  const close=()=>{
  ...
  document.addEventListener('keydown',onKey);
  wrap.addEventListener('click',e=>{if(e.target===wrap)close();});
  wrap.querySelector('#obj-soc-close-x')?.addEventListener('click',close);
  wrap.querySelector('#obj-soc-cancel')?.addEventListener('click',close);
  wrap.querySelector('#obj-soc-save')?.addEventListener('click',()=>{
    const v=document.getElementById('obj-soc-ca')?.value;
    const n=Math.max(0,Number(String(v).replace(/\s/g,'').replace(',','.'))||0);
    ent.months[monthKey]=n;
    next[socId]=ent;
    saveCrmSocieteObjectifs(next);
    renderLeadsDashboard();
    logActivity(`${currentUser?.name||'?'} a fixé l’objectif CA HT ${label} (${labelMois}) : ${n.toLocaleString('fr-FR')} €`);
    close();
  });
  document.body.appendChild(wrap);
  const inp=document.getElementById('obj-soc-ca');
  if(inp){inp.focus();try{inp.select();}catch(e){}}
}

// Champs prénom / nom (stockage combiné dans lead.nom : « Prénom Nom »)
...
function openNewLead(){
  ...
  const actWrapNew=document.getElementById('lead-action-wrap');
...
function getLeadModalSubtitleHtml(l,role){
  if(role==='assistante'){
    return`<span class="lead-secteur">${getLeadSecteurLabel(l.secteur)}</span><span>${getLeadAge(l)}</span>${l.vu_date?`<span class="lead-vu">👁️ Vu le ${l.vu_date}</span>`:''}`;
  }
  const rdvLeadCount=getLeadRdvDoneCount(l);
  const rdvClientCount=getClientRdvDoneCount(l);
  let tail='';
  if(rdvLeadCount!==rdvClientCount)tail+=`<span style="color:var(--t2)">Fiche actuelle: ${rdvLeadCount}</span>`;
  if(l.vu_date)tail+=`<span class="lead-vu">👁️ Vu le ${l.vu_date}</span>`;
  else if(role==='admin'||isCRMScopePilotageRole(role))tail+='<span class="lead-non-vu">👁️ Non ouvert</span>';
  return`<span class="lead-secteur">${getLeadSecteurLabel(l.secteur)}</span>
...
function openLead(id){
  ...
  document.getElementById('modal-lead-sub').innerHTML=getLeadModalSubtitleHtml(l,role);
  resetLeadForm();
  // Remplir champs
  ...
  applyLeadModalSecteurOptions(false);
  const allowedSect=getLeadFormSecteurOptionIds();
  const sv=l.secteur||'nimes';
  ...
  // Commercial supprimé : conserver l’uid sur le lead pour l’historique (libellé via resolveCrmCommercialLabel)
  // Bouton réouvrir si archivé (manuel) ou hors année en cours
  const btnSave=document.getElementById('btn-save-lead');
  if(isLeadCrmArchivedView(l)&&btnSave){
  ...
  // Timeline (réservée admin + dir. commercial + DG — pas les commerciaux)
  if(canViewLeadFullTimeline()&&l.timeline&&l.timeline.length>0){
...
function selectStatut(statut,btn,silent=false){
  ...
  if(statut==='vert'&&!silent){
    const devisEl=document.getElementById('lead-montant-devis');
    const venduEl=document.getElementById('lead-prix-vendu');
    if(devisEl&&venduEl){
      const m=String(devisEl.value||'').replace(/\s/g,'').replace(',','.');
      const v=String(venduEl.value||'').replace(/\s/g,'').replace(',','.');
      const mNum=Number(m);
      if(m&&Number.isFinite(mNum)&&mNum>0&&(!v||!Number.isFinite(Number(v))||Number(v)<=0))venduEl.value=String(mNum);
    }
  }
...
function addLeadAction(){
  if(!canViewLeadFullTimeline())return;
...
async function saveLead(){
  ...
  const nomRaw=getLeadNomCombinedRawFromForm();
  ...
  if(isCrmSalesActorRole(currentUser?.role)&&sectorState.outOfSector&&!justifHorsSecteur){
  ...
  if(currentUser?.role!=='assistante'&&currentLeadStatut==='rouge'&&!raisonMort)missing.push('Raison du refus');
  if(currentUser?.role!=='assistante'&&currentLeadStatut==='vert'){
  ...
  if(currentUser?.role==='assistante'&&currentLeadId){
    const idxAs=leads.findIndex(l=>l.id===currentLeadId);
    if(idxAs!==-1){
      const oldAs=leads[idxAs];
      Object.assign(data,{
        statut:oldAs.statut,
        sous_statut:oldAs.sous_statut||'a_rappeler',
        rappel:oldAs.rappel||null,
        date_rdv_fait:oldAs.date_rdv_fait||null,
        montant_rdv:oldAs.montant_rdv||null,
        montant_devis:oldAs.montant_devis||null,
        date_devis:oldAs.date_devis||null,
        rappel_devis:oldAs.rappel_devis||null,
        prix_vendu:oldAs.prix_vendu||null,
        date_signature:oldAs.date_signature||null,
        produit_vendu:oldAs.produit_vendu||null,
        raison_mort:oldAs.raison_mort||null,
        commercial:oldAs.commercial||null,
        commercial_user_id:oldAs.commercial_user_id||null
      });
    }
  }
  if(currentUser?.role==='assistante'&&!currentLeadId){
  ...
  if(currentLeadId){
    ...
    if(idx!==-1){
      ...
      if(old.statut!==data.statut){
        addLeadTimelineEntry(old,`Statut : ${getLeadStatutPlainLabel(old.statut)} → ${getLeadStatutPlainLabel(data.statut)}`,currentUser.name);
      }
      ...
      if(old.commercial!==data.commercial&&data.commercial){
        addLeadTimelineEntry(old,`Attribué à ${resolveCrmCommercialLabel(data.commercial)}`,currentUser.name);
        // Notifier le commercial
        pushLeadCrmNotif('Lead attribué',`${nom} — ${projet}`,LEAD_SOURCE_ICONS[data.source]||'📋',data.commercial);
      }
      const updatedLead={
        ...old,
        ...data,
    ...
    if(isCrmSalesActorRole(currentUser?.role)&&dirsLead.length){
      dirsLead.forEach(u=>{
        pushLeadCrmNotif('Nouveau lead équipe',`${nom} — ${projet} · ${srcHuman} · Par ${currentUser.name}`,leadSrcIcon,u.id);
      });
...
async function rouvrirLead(id){
  if(!await benaiConfirm('Réouvrir ce lead ?'))return;
  const leads=getLeads();
  const idx=leads.findIndex(l=>l.id===id);if(idx===-1)return;
  leads[idx].archive=false;
  const y=getLeadCreationYearValue(leads[idx]);
  const curY=new Date().getFullYear();
  if(y!==null&&y<curY){
    leads[idx].date_creation=new Date().toISOString();
    addLeadTimelineEntry(leads[idx],`Réouvert — remis dans le pipeline (${curY}, nouvelle date de création)`,currentUser.name);
  }else{
...
async function deleteLead(){
  if(currentUser.id!=='benjamin'){await benaiAlert('Suppression réservée à Benjamin');return;}
  if(!await benaiConfirm('Supprimer ce lead définitivement ?'))return;
  const leads=getLeads();
...
function buildCrmPilotageSecteurFilter(){
  const el=document.getElementById('crm-filter-secteur');
  if(!el)return;
  const ids=getCrmSecteurIdsForScopeSlug(getCrmSecteurScopeSlug());
  el.innerHTML='<option value="">Tous secteurs</option>'+ids.map(id=>`<option value="${id}">${CRM_SECTOR_LABELS[id]||id}</option>`).join('');
}
function applyLeadModalSecteurOptions(preserveValue){
  const sel=document.getElementById('lead-secteur');
  if(!sel)return;
  const ids=getLeadFormSecteurOptionIds();
  const prev=preserveValue?String(sel.value||''):'';
  sel.innerHTML=ids.map(id=>`<option value="${id}">${CRM_SECTOR_LABELS[id]||id}</option>`).join('');
  const fallback=ids[0]||'nimes';
...
function getDuplicateLeadClustersForPilotage(leadsIn){
  const base=Array.isArray(leadsIn)?leadsIn:getCompanyScopedLeads(getLeads().filter(l=>!l._deleted&&!isLeadCrmArchivedView(l)));
  const byKey=new Map();
  base.forEach(l=>{
    const k=getLeadIdentityKey(l.nom,l.adresse,l.cp);
    if(!k)return;
...
function formatDuplicatePilotageBannerHtml(clusters){
  if(!clusters?.length)return'';
  const lines=clusters.slice(0,4).map(arr=>{
    const names=arr.map(l=>esc(l.nom||'Sans nom')).join(' · ');
    return`<div style="font-size:12px;color:var(--t2);margin-top:4px">→ ${names}</div>`;
  }).join('');
  const more=clusters.length>4?`<div style="font-size:11px;color:var(--t3);margin-top:4px">… et ${clusters.length-4} autre(s)</div>`:'';
  return`<div class="sav-form" style="margin:0 0 12px;border-color:rgba(251,191,36,.5);background:rgba(251,191,36,.08)">
    <div style="font-size:13px;font-weight:700;color:var(--y)">⚠️ Doublons probables (même nom + même code postal + même adresse)</div>
    <div style="font-size:12px;color:var(--t2);line-height:1.45;margin-top:4px">Les assistantes ne vérifient pas les doublons à la saisie : ouvrez les fiches ci-dessous et fusionnez ou archivez si besoin.</div>
    ${lines}${more}
  </div>`;
}
...
/** CP : recalcul secteur (assistante = toujours auto depuis le CP). */
function onLeadCpInputAssistanteSync(){
  const raw=document.getElementById('lead-cp')?.value||'';
  if(currentUser?.role==='assistante')detectSecteurByCP(raw);
  updateLeadSectorSignals();
}
function applyLeadModalAssistanteUi(){
  const hint=document.getElementById('lead-secteur-assistante-hint');
  const sel=document.getElementById('lead-secteur');
  const isAs=currentUser?.role==='assistante';
  if(hint)hint.style.display=isAs?'inline':'none';
  if(sel)sel.disabled=!!isAs;
}
...
function fillCommercialAssign(selected=''){
  ...
  const users=getAllUsers().filter(u=>{
    ...
    if(isCRMScopePilotageRole(currentUser?.role)){
      ...
      const us=String(u.societe||'').trim().toLowerCase();
      if(cs==='les-deux')return true;
      if(us==='les-deux')return true;
      if(cs==='lambert')return us==='lambert';
      return us!=='lambert';
    }
    return true;
  });
...
function isLeadAlerte(l){
  if(l.statut==='vert'||l.statut==='rouge'||isLeadCrmArchivedView(l))return false;
...
function exportLeadsExcel(){
  const leads=getCompanyScopedLeads(getLeads());
  const header=joinCsv(['Date','Source','Nom','Téléphone','Ville','CP','Projet','Statut','Commercial','Secteur','Devis (€)','CA (€)','Suivi','Commentaire'])+'\n';
  ...
  downloadCsv('Leads_BenAI_'+new Date().toLocaleDateString('fr-FR').replace(/\//g,'-')+'.csv',header+rows);
...
function buildSecteurSynthDataRows(actifs,ids){
  return ids.map(s=>{
    ...
    return joinCsv([csv(getLeadSecteurLabel(s)),sl.length,ventes.length,ca,devis.length,pipeline]);
  }).join('\n');
}
...
function buildSecteurSynthCSVString(actifs,ids,withTotalRow){
  const header=joinCsv(['Secteur','Leads actifs','Ventes','CA signé (€)','Devis en cours','Pipeline devis (€)'])+'\n';
  ...
  const ca=ventes.reduce((sum,l)=>sum+Number(l.prix_vendu||0),0);
  const pipeline=devis.reduce((sum,l)=>sum+Number(l.montant_devis||0),0);
  const totalLine=joinCsv([csv('TOTAL (sélection)'),allSl.length,ventes.length,ca,devis.length,pipeline]);
  return header+body+'\n'+totalLine;
}

function exportSecteurSyntheseFromDashboard(){
  const sel=(document.getElementById('crm-dash-export-secteur')?.value)||'all';
  const actifs=getCompanyScopedLeads(getLeads()).filter(l=>!isLeadCrmArchivedView(l));
  const stamp=new Date().toISOString().slice(0,10);
  const scopeLabel=isCRMScopePilotageRole(currentUser?.role)?`scope_${currentUser.societe||'societe'}`:'scope_global';
  const scopedIds=getCrmSecteurIdsForScopeSlug(getCrmSecteurScopeSlug());
  if(sel==='all'){
    const fname=`CRM_Secteurs_${scopeLabel}_${stamp}.csv`;
    const content=buildSecteurSynthCSVString(actifs,scopedIds,scopedIds.length>1);
    downloadCsv(fname,content);
    showDriveNotif('📥 Export secteur prêt');
    logActivity(`${currentUser?.name||'?'} a exporté synthèse secteurs (périmètre)`);
    return;
  }
  let fname,content;
  if(sel==='core3'){
    const cluster=getCrmSecteurCoreIdsInScope();
    fname=`CRM_Secteurs_horsZB_${scopeLabel}_${stamp}.csv`;
    content=buildSecteurSynthCSVString(actifs,cluster.length?cluster:CRM_SECTEUR_IDS_CORE3,cluster.length>1);
  }else{
...
function buildCRMExportBundles(){
  ...
  const leadsHeader=joinCsv(['Date','Source','Nom','Téléphone','Ville','CP','Projet','Statut','Commercial','Secteur','Hors secteur','Zone blanche','Devis (€)','CA (€)','Suivi','Commentaire'])+'\n';
  ...
  const secteurRows=buildSecteurSynthDataRows(actifs,getCrmSecteurIdsForScopeSlug(getCrmSecteurScopeSlug()));

  const srcHeader=joinCsv(['Mois','Source','Leads','Ventes','CA signé (€)'])+'\n';
  const srcRows=LEAD_SOURCE_CODES.map(src=>{
    const sl=leadsMonth.filter(l=>l.source===src);
    const ventes=sl.filter(l=>l.statut==='vert');
    const ca=ventes.reduce((sum,l)=>sum+Number(l.prix_vendu||0),0);
    return joinCsv([currentMonth,csv(LEAD_SOURCE_LABELS[src]||src),sl.length,ventes.length,ca]);
  }).join('\n');

  const ranking=getCommercialRankingMonth(actifs);
  const rankHeader=joinCsv(['Mois','Rang','Commercial','Ventes','CA signé (€)'])+'\n';
  const rankRows=ranking.map((r,i)=>{
    return joinCsv([currentMonth,i+1,csv(resolveCrmCommercialLabel(r.uid)),r.ventes,r.ca]);
  }).join('\n');

  const lost=getLeadLostAnalytics(leadsMonth);
  const lostHeader=joinCsv(['Mois','Motif','Leads perdus'])+'\n';
  const lostRows=(lost.rows||[]).map(r=>joinCsv([currentMonth,csv(r.label),r.count])).join('\n');
...
function exportAllCRMTables(){
  if(window._benaiExportAllBusy){
    showDriveNotif('📦 Export déjà en cours — patientez une seconde');
    return;
  }
  const payload=buildCRMExportBundles();
  const items=Object.values(payload.bundles||{});
  if(!items.length){
    showDriveNotif('⚠️ Aucun export à lancer');
    return;
  }
  window._benaiExportAllBusy=true;
  items.forEach((item,i)=>{
    setTimeout(()=>{
      downloadCsv(item.filename,item.content);
      if(i===items.length-1){
        window._benaiExportAllBusy=false;
...
function setupSafeSelfHealing(){
  ...
  document.addEventListener('visibilitychange',()=>{
    if(document.visibilityState==='visible')lastPollingHeartbeat=Date.now();
  });
  setInterval(async()=>{
    if(!currentUser)return;
    // Si le polling est tombé, on le relance automatiquement.
    const staleMs=Date.now()-lastPollingHeartbeat;
    const pollingMissing=!pollingInterval;
    const pollingStale=document.visibilityState==='visible'&&staleMs>POLLING_STALE_MS;
    if(pollingMissing||pollingStale){
      startPolling();
      reportAutoBug('BenAI IA','Auto-correction: polling relancé automatiquement.','mineur',{autoFixed:true,staleMs,visible:document.visibilityState==='visible'});
...
function renderBugsList(){
  ...
  else if(currentBugFilter==='bloquant')bugs=bugs.filter(b=>b.gravite==='bloquant'&&b.statut==='ouvert');
...
function resolveBug(id){
  ...
  saveBugs(bugs);
  currentBugFilter='ouvert';
  document.querySelectorAll('#bug-filter-bar .crm-filter').forEach(x=>x.classList.remove('active'));
  const ouvBtn=document.querySelector('#bug-filter-bar [data-bug-filter="ouvert"]');
  if(ouvBtn)ouvBtn.classList.add('active');
...
// ══════════════════════════════════════════
// 🎯 OBJECTIFS COMMERCIAUX (CA mensuel par mois YYYY-MM + CA hebdo global)
// ══════════════════════════════════════════
function getDefaultObjCommMonthKey(){
  const d=new Date();
  return`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`;
}
function normalizeCommercialObjEntry(v){
  if(!v||typeof v!=='object')return{hebdo:0,months:{}};
  if(v.months&&typeof v.months==='object'&&!Array.isArray(v.months)){
    const months={};
    Object.keys(v.months).forEach(k=>{
      const m=v.months[k];
      const weeksRaw=m?.weeks&&typeof m.weeks==='object'&&!Array.isArray(m.weeks)?m.weeks:{};
      const weeks={};
      Object.keys(weeksRaw).forEach(wk=>{
        weeks[wk]=Number(weeksRaw[wk])||0;
      });
      months[k]={mensuel:Number(m?.mensuel)||0,weeks};
    });
    return{hebdo:Number(v.hebdo)||0,months};
  }
  const d=new Date();
  const k=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`;
  return{hebdo:Number(v.hebdo)||0,months:{[k]:{mensuel:Number(v.mensuel)||0,weeks:{}}}};
}
function getObjCommMonthOptions(){
  const keys=new Set();
  const now=new Date();
  ...
  for(let m=1;m<=12;m++){
    ...
    keys.add(`${yPrev}-${String(m).padStart(2,'0')}`);
  }
  try{
    getCompanyScopedLeads(getLeads()).filter(l=>l.statut==='vert').forEach(l=>{
      const d=new Date(l.date_creation||0);
      if(isNaN(d.getTime()))return;
      const yy=d.getFullYear();
      if(yy===y||yy===yPrev)keys.add(`${yy}-${String(d.getMonth()+1).padStart(2,'0')}`);
    });
  }catch(e){}
  return[...keys].sort((a,b)=>b.localeCompare(a,'fr'));
}
function getObjectifsCommerciaux(){
  try{
    const raw=JSON.parse(appStorage.getItem('benai_obj_comm'))||{};
    let migrated=false;
    const out={};
    Object.keys(raw).forEach(uid=>{
      const v=raw[uid];
      if(v&&typeof v==='object'&&v.months&&typeof v.months==='object'&&!Array.isArray(v.months)){
        out[uid]=normalizeCommercialObjEntry(v);
      }else{
        migrated=true;
        out[uid]=normalizeCommercialObjEntry(v||{});
      }
    });
    if(migrated)saveObjectifsCommerciaux(out);
    return out;
  }catch{return{};}
}
function saveObjectifsCommerciaux(o){appStorage.setItem('benai_obj_comm',JSON.stringify(o));}

function setObjectifCommercial(uid,monthKey,mensuel,hebdo,weeks){
  const all=getObjectifsCommerciaux();
  const prev=normalizeCommercialObjEntry(all[uid]||{});
  const parseNum=x=>{
    const s=String(x==null?'':x).replace(/\s/g,'').replace(',','.');
    const n=Number(s);
    return Number.isFinite(n)?n:0;
  };
  prev.hebdo=parseNum(hebdo);
  if(!prev.months)prev.months={};
  const prior=prev.months[monthKey]||{};
  const wk=weeks&&typeof weeks==='object'&&!Array.isArray(weeks)?weeks:(prior.weeks&&typeof prior.weeks==='object'?prior.weeks:{});
  prev.months[monthKey]={mensuel:parseNum(mensuel),weeks:wk};
  all[uid]=prev;
  saveObjectifsCommerciaux(all);
}

function getProgressionCommercial(uid,forMonthKey){
  const monthKey=forMonthKey||getDefaultObjCommMonthKey();
  const parts=monthKey.split('-');
  const year=Number(parts[0]),monthNum=Number(parts[1]);
  const monthIdx=monthNum-1;
  if(!Number.isFinite(year)||!Number.isFinite(monthIdx)||monthIdx<0||monthIdx>11){
    return{ventes_mois:0,ventes_semaine:0,ca_mois:0,ca_semaine:0,obj:{mensuel:0,hebdo:0},_objMonthKey:monthKey,_objWeekKey:''};
  }
  ...
  const now=new Date();
  const curWk=getISOWeekLabelFromDate(now);
  const ventesSem=leads.filter(l=>{
    const d=new Date(l.date_creation||0);
    if(isNaN(d.getTime()))return false;
    return getISOWeekLabelFromDate(d)===curWk;
  });
  const caMois=ventesMois.reduce((s,l)=>s+Number(l.prix_vendu||0),0);
  const caSem=ventesSem.reduce((s,l)=>s+Number(l.prix_vendu||0),0);
  const entry=normalizeCommercialObjEntry(getObjectifsCommerciaux()[uid]||{});
  const objMensuel=entry.months[monthKey]?.mensuel??0;
  const wkMap=entry.months[monthKey]?.weeks;
  let objHebdo=Number(wkMap?.[curWk]||0);
  if(!objHebdo)objHebdo=Number(entry.hebdo)||0;
  return{
    ventes_mois:ventesMois.length,
    ventes_semaine:ventesSem.length,
    ca_mois:caMois,
    ca_semaine:caSem,
    obj:{mensuel:objMensuel,hebdo:objHebdo},
    _objMonthKey:monthKey,
    _objWeekKey:curWk
  };
}
...
function getISOWeekLabelFromDate(d0){
  const d=new Date(d0);
  if(isNaN(d.getTime()))return'';
  d.setHours(0,0,0,0);
  d.setDate(d.getDate()+3-(d.getDay()+6)%7);
  const isoY=d.getFullYear();
  const w=getWeekNumber(d0);
  return`${isoY}-W${String(w).padStart(2,'0')}`;
}
function collectIsoWeekKeysInMonth(year,month1to12){
  const keys=[];
  const seen=new Set();
  const last=new Date(year,month1to12,0).getDate();
  for(let day=1;day<=last;day++){
    const k=getISOWeekLabelFromDate(new Date(year,month1to12-1,day));
...
async function genererMessageMotivation(uid,contexte){
  ...
  const user=getAllUsers().find(u=>String(u.id)===String(uid));
  if(!user)return;
  const pctMois=prog.obj.mensuel>0?Math.round(prog.ca_mois/prog.obj.mensuel*100):0;
  const histKey='benai_motiv_hist_'+String(uid);
  let hist=[];
  try{
    hist=JSON.parse(appStorage.getItem(histKey)||'[]');
    if(!Array.isArray(hist))hist=[];
  }catch{hist=[];}
  const histTxt=hist.slice(-14).map(t=>`- ${String(t).slice(0,200).replace(/\s+/g,' ').trim()}`).join('\n')||'- (aucun historique)';
  const angles=['closing et clarté client','constance sur le terrain','fierté du progrès mesurable','énergie sur la dernière ligne droite','discipline CRM (dates, montants, suivis)','confiance sur le prochain rendez-vous'];
  const angle=angles[Math.floor(Math.random()*angles.length)];
  const jour=new Date().toLocaleDateString('fr-FR',{weekday:'long'});
  const prompt=`Tu es BenAI, coach commercial interne. Génère un message court (2 phrases max, 3 seulement si indispensable), en français, pour ${user.name}.
Contexte situationnel : ${contexte}
Données réelles à intégrer naturellement (obligatoire) : ${prog.ventes_mois} vente(s) signée(s) ce mois, CA ${prog.ca_mois.toLocaleString('fr-FR')}€ HT, objectif mensuel ${prog.obj.mensuel.toLocaleString('fr-FR')}€ (${pctMois}% atteint), ${prog.ventes_semaine} vente(s) cette semaine ISO${prog._objWeekKey?' ('+prog._objWeekKey+')':''}, cible hebdo affichée ${prog.obj.hebdo.toLocaleString('fr-FR')}€ HT.
Aujourd'hui : ${jour}. Angle d'écriture imposé : ${angle}.
Contraintes : tutoie cette personne ; ton chaleureux mais crédible ; zéro cliché LinkedIn ; pas de liste à puces ; pas de « champion » ni « machine » ni « warrior ».
ANTI-RÉPÉTITION : tu dois être différent de tout message déjà envoyé. Ne réutilise ni les mêmes phrases ni les mêmes ouvertures ni les mêmes images que l’historique ci-dessous (même si c’était bien) — varie structure, verbes, rythme.
Historique des messages récents (à ne pas imiter ni paraphraser) :
${histTxt}`;
  try{
    const res=await fetch('https://api.anthropic.com/v1/messages',{
      method:'POST',
    ...
    if(data.content?.[0]?.text){
      ...
      hist.push(msg.slice(0,500));
      if(hist.length>36)hist=hist.slice(-36);
      appStorage.setItem(histKey,JSON.stringify(hist));
...
function checkMotivationsAuto(){
  ...
  const nm=String(currentUser.name||'').trim().split(/\s+/)[0]||'toi';
  let contexte='';
  if(pct>=1){
    const opts=[
      `Le plafond du mois est déjà dépassé pour ${nm} : souligne ce que ça implique pour la dynamique d’équipe et la qualité du pipe.`,
      `${nm} a dépassé l’objectif mensuel : insiste sur la régularité (CRM, relances) pour capitaliser sans se disperser.`,
      `CA du mois au-dessus de la cible pour ${nm} : message centré sur la suite (consolider, sécuriser les signatures, anticiper le mois suivant).`
    ];
    contexte=opts[Math.floor(Math.random()*opts.length)];
  }else if(pct>=0.75){
    const opts=[
      `${nm} est dans la zone 75–100 % de l’objectif : parle priorisation des dossiers chauds et d’une fin de mois maîtrisée.`,
      `Dernière ligne droite objectif pour ${nm} : insiste sur les relances devis et la clarté des prochaines actions.`,
      `Encore un effort mesurable pour ${nm} pour boucler le mois : message ancré sur le terrain (RDV, devis, suivi).`
    ];
    contexte=opts[Math.floor(Math.random()*opts.length)];
  }else if(new Date().getDay()===1){
    const opts=[
      `Lundi : ${nm} repart sur une semaine neuve — message sur rythme, intention de la semaine, et une victoire petite mais concrète à viser.`,
      `Début de semaine pour ${nm} : propose un focus unique (ex. relances devis ou RDV) sans moraliser.`,
      `Semaine qui démarre pour ${nm} : encourage la relecture du pipe et un premier « quick win » avant mercredi.`
    ];
    contexte=opts[Math.floor(Math.random()*opts.length)];
  }else if(prog.ventes_semaine===0&&new Date().getDay()>=3){
    const opts=[
      `${nm} n’a pas encore signé cette semaine : message sur relances courtes, créneaux téléphoniques, et mise à jour CRM (sans culpabiliser).`,
      `Semaine encore vide en signature pour ${nm} : parle opportunités proches (devis en attente, RDV à confirmer).`,
      `Pas de vente cette semaine pour l’instant pour ${nm} : insiste sur le contrôle du pipeline et une action terrain simple aujourd’hui.`
    ];
    contexte=opts[Math.floor(Math.random()*opts.length)];
  }else return;
...
function renderTableauVentes(container){
  ...
  const yPrev=yNow-1;
  const fromSales=[...new Set(leads.map(l=>{const d=new Date(l.date_creation||0);return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`}))].filter(k=>{
    const y=Number(String(k).split('-')[0]);
    return Number.isFinite(y)&&(y===yNow||y===yPrev);
  });
  let moisDispos=[...new Set([...fromSales,...getObjCommMonthOptions()])].sort((a,b)=>b.localeCompare(a,'fr'));
  if(!moisDispos.length)moisDispos=[currentKey];
  let selectedMois=appStorage.getItem('benai_ventes_mois')||currentKey;
  if(!moisDispos.includes(selectedMois)){
    selectedMois=currentKey;
    appStorage.setItem('benai_ventes_mois',selectedMois);
  }
  ...
  if(Number.isFinite(yr)&&Number.isFinite(mo)&&mo>=1&&mo<=12){
    if(yr===yNow){
      const prevKey=`${yPrev}-${String(mo).padStart(2,'0')}`;
      const prevLeads=leads.filter(l=>{const d=new Date(l.date_creation||0);return d.getFullYear()===yPrev&&d.getMonth()+1===mo;});
      const prevTot=prevLeads.reduce((s,l)=>s+Number(l.prix_vendu||0),0);
      compareLine=`<div style="font-size:12px;color:var(--t2);margin:-6px 0 12px;line-height:1.45">↩️ <strong>N-1</strong> (${formatMois(prevKey)}) : <strong style="color:var(--t1)">${prevTot.toLocaleString('fr-FR')} €</strong> HT · ${prevLeads.length} vente(s) — même mois, année précédente.</div>`;
    }else if(yr===yPrev){
      const curKey=`${yNow}-${String(mo).padStart(2,'0')}`;
      const curLeads=leads.filter(l=>{const d=new Date(l.date_creation||0);return d.getFullYear()===yNow&&d.getMonth()+1===mo;});
      const curTot=curLeads.reduce((s,l)=>s+Number(l.prix_vendu||0),0);
      compareLine=`<div style="font-size:12px;color:var(--t2);margin:-6px 0 12px;line-height:1.45">↪️ <strong>Année en cours</strong> (${formatMois(curKey)}) : <strong style="color:var(--t1)">${curTot.toLocaleString('fr-FR')} €</strong> HT · ${curLeads.length} vente(s) — même mois.</div>`;
    }
  }
  ...
  const thead=ventesTableCommercialOnly
    ?`<thead><tr style="background:var(--s3)">
          ...
          <th style="padding:7px 10px;text-align:right;color:var(--t3);font-size:10px;text-transform:uppercase">${thPrix}</th>
        </tr></thead>`
    :`<thead><tr style="background:var(--s3)">
  ...
  const tbody=leadsFiltered.map(l=>{
    if(ventesTableCommercialOnly){
      return`<tr style="border-top:1px solid var(--b1)">
              <td style="padding:8px 10px;font-weight:500">${esc(l.nom)}</td>
  ...
  const footColspan=ventesTableCommercialOnly?2:4;
  const html=`
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
      ...
      <div style="display:flex;gap:8px;align-items:center">
        ...
        ${canExportSales?`<button onclick="exportTableauVentes()" style="padding:6px 12px;background:var(--g2);color:var(--g);border:1px solid rgba(34,197,94,.3);border-radius:8px;font-size:11px;font-weight:600;cursor:pointer;font-family:inherit">📥 Export</button>`:''}
      </div>
    </div>
    <div style="font-size:22px;font-weight:800;color:var(--g);margin-bottom:4px">${total.toLocaleString('fr-FR')} € HT <span style="font-size:12px;color:var(--t3);font-weight:400">${leadsFiltered.length} ventes</span></div>
    ${compareLine}
    <div style="overflow-x:auto">
      <table style="width:100%;border-collapse:collapse;font-size:12px">
        ...
        <tfoot><tr style="background:var(--a3);border-top:2px solid var(--a)">
          <td colspan="${footColspan}" style="padding:8px 10px;font-weight:700;color:var(--a)">TOTAL</td>
...
function exportTableauVentes(){
  ...
  const currentKey=`${yNow}-${String(now.getMonth()+1).padStart(2,'0')}`;
  let selectedMois=appStorage.getItem('benai_ventes_mois')||currentKey;
  const py=Number(String(selectedMois).split('-')[0]);
  if(!Number.isFinite(py)||(py!==yNow&&py!==yPrev)){selectedMois=currentKey;appStorage.setItem('benai_ventes_mois',selectedMois);}
  const [yr,mo]=selectedMois.split('-').map(Number);
...
// 🔒 BLOQUER COMMERCIAL
// ══════════════════════════════════════════
async function toggleBlockCommercial(uid){
  if(!canManageBenaiUsersAdmin()){await benaiAlert('Réservé à l’administrateur.');return;}
  const access=getAccess();
  const u=getAllUsers().find(x=>x.id===uid);
  const isBlocked=access[uid]===false;
  if(!await benaiConfirm(`${isBlocked?'Débloquer':'Bloquer'} ${u?.name||uid} du CRM ?`))return;
...
async function confirmerFusion(){
  ...
  if(!await benaiConfirm('Fusionner ces 2 leads ? Le premier sera conservé, le second supprimé.'))return;
  const leads=getLeads();
...
/** Excel (liste FR) : point-virgule ; + BOM UTF-8 → colonnes correctes et accents (é, €) au double-clic. */
const CSV_LIST_SEP=';';
const CSV_UTF8_BOM='\uFEFF';
function joinCsv(parts){return parts.join(CSV_LIST_SEP);}
function downloadCsv(filename,content){
  download(filename,CSV_UTF8_BOM+content,'text/csv;charset=utf-8');
}
...
function getNotifs(){
  try{
    const arr=JSON.parse(appStorage.getItem('benai_notifs_'+(currentUser?.id||'?')))||[];
    return arr.map(n=>({...n,read:n.read===false?false:true}));
  }catch{return[];}
}
...
function refreshNotifBadge(){
  ...
  if(unread>0){
    ...
    dot.style.animation='pulse-notif .8s infinite';
  } else {
...
function renderNotifList(){
  ...
  list.innerHTML=notifs.map(n=>`
    <div class="notif-item${n.read===false?' unread':''}" onclick="readNotif(${n.id})">
...
window.addEventListener('load',async()=>{
  ...
  registerBenAISW();
  refreshInstallButton();
  // Thème (clair par défaut ; sombre seulement si benai_theme === 'dark')
  applyTheme();
  syncSettingsThemeButtons();
...
/** Ne pas écraser spellcheck/autocorrect explicites (SAV, CRM, login…) ni les champs techniques. */
function benaiSkipGlobalSpellcheck(el){
  if(!el)return true;
  if(el.getAttribute('data-benai-no-spell')==='1')return true;
  if(el.closest&&el.closest('[data-benai-no-spell="1"]'))return true;
  if(el.hasAttribute('spellcheck'))return true;
  const t=String(el.type||'').toLowerCase();
  if(['email','password','url','tel','number','date','datetime-local','month','week','time'].includes(t))return true;
  if(t==='search')return true;
  return false;
}
function benaiApplySpellcheckPolicy(el,isPhone){
  if(benaiSkipGlobalSpellcheck(el))return;
    if(isPhone){
      el.spellcheck=false;
...
function enableSpellcheckEverywhere(){
  ...
  document.querySelectorAll('input[type="text"],input[type="search"],textarea').forEach(el=>{
    if(el.hasAttribute('data-benai-spell-applied'))return;
    el.setAttribute('data-benai-spell-applied','1');
    benaiApplySpellcheckPolicy(el,isPhone);
  });
  let spellMoTimer=null;
  new MutationObserver(()=>{
    if(spellMoTimer)return;
    spellMoTimer=requestAnimationFrame(()=>{
      spellMoTimer=null;
      document.querySelectorAll('input[type="text"],input[type="search"],textarea').forEach(el=>{
        if(el.hasAttribute('data-benai-spell-applied'))return;
        el.setAttribute('data-benai-spell-applied','1');
...
try{
  const _syncCorrectBtn=()=>{
    ...
    if(btn&&document.getElementById('app')?.classList.contains('visible')){
      btn.style.display=isBenAIDesktopAutocorrect()?'inline-flex':'none';
    }
  };
  ['(min-width: 641px)','(pointer: fine)'].forEach(q=>{
    const m=window.matchMedia(q);
    const fn=()=>_syncCorrectBtn();
...
function getRoleTutoSlides(role){
  let slides;
  if(role==='directeur_co')slides=getDirecteurCoTutoSlides();
  else if(role==='directeur_general')slides=getDirecteurGeneralTutoSlides();
  else slides=TUTO_SLIDES_BY_ROLE[role]||TUTO_SLIDES_BY_ROLE.default;
...
function renderTutoSlide(){
  ...
  if(tutoStep===currentTutoSlides.length-1){
    btn.textContent=currentUser&&shouldForceRoleGuide()?'Terminer → valider le Guide':'Commencer 🚀';
  } else {
...
function closeTuto(){
  ...
  setTimeout(()=>{
    try{
      if(currentUser&&shouldForceRoleGuide()){
        showDriveNotif('📘 Dernière étape : ouvrez l’onglet **Guide** puis cliquez sur « J’ai lu et je valide » pour débloquer tout BenAI.');
        showPage('guide');
      }
    }catch(_){}
  },350);
